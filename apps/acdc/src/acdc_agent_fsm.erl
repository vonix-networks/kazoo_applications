...
-record(state, {
    ...
    % optionally set on win
    wrapup_timeout = 0 :: integer(),
    ...
    % used when no call-ids are available
    fsm_call_id :: kz_term:api_binary(),
...
call_event(ServerRef, <<"call_event">>, <<"LEG_CREATED">>, JObj) ->
    ...
    case kz_json:get_ne_binary_value(<<"Channel-Loopback-Leg">>, JObj) of
        'undefined' ->
            gen_statem:cast(
                ServerRef, {'leg_created', call_id(JObj), kz_call_event:other_leg_call_id(JObj)}
            );
        _ ->
            'ok'
    end;
...
maybe_send_execute_complete(ServerRef, <<"bridge">>, JObj) ->
    lager:info(
        "send EXECUTE_COMPLETE,bridge to ~p with ci: ~s, olci: ~s",
        [
            ServerRef,
            call_id(JObj),
...
maybe_send_execute_complete(_, _, _) ->
    'ok'.
...
originate_uuid(ServerRef, JObj) ->
    gen_statem:cast(
        ServerRef,
        {'originate_uuid', kz_json:get_value(<<"Outbound-Call-ID">>, JObj),
...
start_link(Supervisor, ThiefCall, _QueueId) ->
    pvt_start_link(
        kapps_call:account_id(ThiefCall),
...
-spec start_link(pid(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    kz_types:startlink_ret().
start_link(Supervisor, AccountId, AgentId, AgentJObj) ->
    start_link(Supervisor, AccountId, AgentId, AgentJObj, []).

-spec start_link(
    pid(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binaries()
) -> kz_types:startlink_ret().
...
wait_for_listener(Supervisor, ServerRef, Props, IsThief) ->
    case acdc_agent_sup:listener(Supervisor) of
        ...
        P when is_pid(P) ->
            ...
            {NextState, SyncRef} =
                case
                    props:get_value('skip_sync', Props) =:= 'true' orelse
                        IsThief
                of
                    'true' ->
                        {'ready', 'undefined'};
...
sync(
    'info',
    {'timeout', Ref, ?SYNC_RESPONSE_MESSAGE},
    #state{
        sync_ref = Ref,
        agent_listener = AgentListener
    } = State
...
sync('info', {'timeout', Ref, ?RESYNC_RESPONSE_MESSAGE}, #state{sync_ref = Ref} = State) when
    is_reference(Ref)
->
...
sync('info', ?NEW_CHANNEL_FROM(CallId, Number, Name, _), State) ->
    ...
    {'next_state', 'inbound', start_inbound_call_handling(CallId, Number, Name, State),
        'hibernate'};
sync('info', ?NEW_CHANNEL_TO(CallId, Number, Name), State) ->
    lager:debug("sync call_to outbound: ~s", [CallId]),
    {'next_state', 'outbound', start_outbound_call_handling(CallId, Number, Name, State),
        'hibernate'};
sync('info', Evt, State) ->
...
ready(
    'cast',
    {'member_connect_win', JObj, 'same_node'},
    #state{
        agent_listener = AgentListener,
        endpoints = OrigEPs,
        account_id = AccountId,
    ...
    case get_endpoints(OrigEPs, Call, AgentId, QueueId) of
        ...
        {'ok', UpdatedEPs} ->
            ...
            NextState =
                case kz_json:get_value(<<"Callback-Details">>, JObj) of
                    'undefined' ->
                        acdc_agent_listener:bridge_to_member(
                            AgentListener, Call, JObj, UpdatedEPs, CDRUrl, RecordingUrl
                        ),
                        'ringing';
                    Details ->
                        acdc_agent_listener:originate_callback_to_agent(
                            AgentListener, Call, JObj, UpdatedEPs, CDRUrl, RecordingUrl, Details
                        ),
                        'ringing_callback'
                end,
            ...
            lager:debug("notifications for the queue: ~p", [
                kz_json:get_value(<<"Notifications">>, JObj)
            ]),
            {'next_state', NextState, State#state{
...
ready(
    'cast',
    {'member_connect_win', JObj, 'different_node'},
    #state{
        agent_listener = AgentListener,
        agent_id = AgentId
    } = State
...
ready(
    'cast',
    {'member_connect_req', _},
    #state{
        max_connect_failures = Max,
        connect_failures = Fails
    } = State
) when is_integer(Max), Fails >= Max ->
...
ready('cast', {'channel_answered', JObj}, #state{outbound_call_ids = OutboundCallIds} = State) ->
    ...
    case lists:member(CallId, OutboundCallIds) of
        'true' ->
            ...
            {'next_state', 'outbound',
                start_outbound_call_handling(CallId, clear_call(State, 'ready')), 'hibernate'};
        'false' ->
...
ready(
    'cast',
    ?DESTROYED_CHANNEL(CallId, _Cause),
    #state{
        agent_listener = AgentListener,
    ...
    case lists:member(CallId, OutboundCallIds) of
        'true' ->
            ...
            {'next_state', 'ready', State#state{
                outbound_call_ids = lists:delete(CallId, OutboundCallIds)
            }};
        'false' ->
...
ringing(
    'cast',
    {'member_connect_satisfied', JObj, Node},
    #state{
        agent_listener = AgentListener,
    ...
    lager:info(
        "received connect_satisfied: check if I should hangup:~p  MemberCall:~p CallId:~p AgentId:~p AcceptedAgentId:~p",
        [Node, MemberCallId, CallId, AgentId, AcceptedAgentId]
    ),
    case
        CallId =:= MemberCallId andalso
            AgentId /= AcceptedAgentId
    of
        true ->
            lager:info("agent ~p hanging up: agent ~p is handling the call", [
                AgentId, AcceptedAgentId
            ]),
...
ringing(
    'cast', {'originate_uuid', ACallId, ACtrlQ}, #state{agent_listener = AgentListener} = State
) ->
...
ringing(
    ...
    {'next_state', 'answered', State#state{
...
ringing(
    ...
    kapi_acdc_agent:publish_shared_originate_failure([
    ...
    case call_id(JObj) of
        ...
        OtherCallId ->
            case lists:member(OtherCallId, OutboundCallIds) of
                'true' ->
                    lager:debug("agent picked up outbound call ~s instead of the queue call ~s", [
                        OtherCallId, MemberCallId
                    ]),
                    acdc_agent_listener:hangup_call(AgentListener),
                    {'next_state', 'outbound',
                        start_outbound_call_handling(OtherCallId, clear_call(State, 'ready')),
                        'hibernate'};
                'false' ->
...
ringing(
    ...
    kapi_acdc_agent:publish_shared_call_id([
        {<<"Account-ID">>, AccountId},
        {<<"Agent-ID">>, AgentId},
...
ringing(
    'info',
    ?NEW_CHANNEL_FROM(AgentCallId, _, _, MemberCallId),
    #state{member_call_id = MemberCallId} = State
) ->
    lager:debug("new channel ~s for agent", [AgentCallId]),
    {'next_state', 'ringing', State#state{agent_call_id = AgentCallId}};
ringing(
    'info',
    ?NEW_CHANNEL_FROM(CallId, Number, Name, _),
    #state{agent_listener = AgentListener} = State
) ->
    lager:debug("ringing call_from inbound: ~s", [CallId]),
    acdc_agent_listener:hangup_call(AgentListener),
    {'next_state', 'inbound',
        start_inbound_call_handling(CallId, Number, Name, clear_call(State, 'ready')), 'hibernate'};
ringing(
    'info',
    ?NEW_CHANNEL_TO(CallId, _, _),
    #state{
        agent_listener = AgentListener,
        outbound_call_ids = OutboundCallIds
    } = State
    ...
    {'next_state', 'ringing', State#state{
        outbound_call_ids = [CallId | lists:delete(CallId, OutboundCallIds)]
    }};
...
ringing(
    'cast',
    ?DESTROYED_CHANNEL(AgentCallId, _Cause),
    #state{
        agent_listener = AgentListener,
...
ringing(
    'cast',
    ?DESTROYED_CHANNEL(MemberCallId, _Cause),
    #state{
        agent_listener = AgentListener,
        account_id = AccountId,
        member_call_id = MemberCallId,
...
ringing_callback(
    'cast', {'shared_call_id', JObj}, #state{agent_callback_call = 'undefined'} = State
) ->
    ACallId = kz_json:get_value(<<"Agent-Call-ID">>, JObj),
...
ringing_callback(
    ...
    {'next_state', 'ringing_callback', State#state{
        agent_call_id = ACallId,
        connect_failures = 0,
...
ringing_callback('cast', {'channel_answered', JObj}, State) ->
    ...
    ACall = kapps_call:set_account_id(
...
ringing_callback(
    ...
    {'next_state', 'awaiting_callback', State#state{
...
ringing_callback(
    ...
    NewMemberCallId = acdc_agent_listener:originate_callback_return(
        AgentListener, AgentCallbackCall
    ),
...
ringing_callback(
    'cast',
    ?DESTROYED_CHANNEL(ACallId, _Cause),
    #state{
        agent_call_id = ACallId,
        connect_failures = Fails,
        max_connect_failures = MaxFails,
...
ringing_callback(
    'cast',
    ?DESTROYED_CHANNEL(ACallId, Cause),
    #state{
        account_id = AccountId,
        agent_id = AgentId,
        agent_listener = AgentListener,
...
ringing_callback(
    'cast',
    ?NEW_CHANNEL_FROM(CallId, Name, Number, MemberCallId),
    #state{member_call_id = MemberCallId} =
        State
) ->
...
ringing_callback({'call', From}, 'status', State) ->
    {'next_state', 'ringing_callback', State, {'reply', From, [{'state', <<"ringing_callback">>}]}};
ringing_callback(
    {'call', From},
    'current_call',
    #state{
...
%% @end
%%------------------------------------------------------------------------------
-spec awaiting_callback(gen_statem:event_type(), any(), state()) ->
...
awaiting_callback(
    'cast',
    {'originate_uuid', MemberCallbackCallId, CtrlQ},
    #state{member_callback_candidates = Candidates} = State
) ->
    lager:debug("recv originate_uuid for member callback call ~s(~s)", [MemberCallbackCallId, CtrlQ]),
    {'next_state', 'awaiting_callback', State#state{
        member_callback_candidates = props:set_value(MemberCallbackCallId, CtrlQ, Candidates)
    }};
...
awaiting_callback(
    ...
    NewMemberCall = kapps_call:exec(
        [
            fun(Call) -> kapps_call:set_account_id(kapps_call:account_id(MemberCall), Call) end,
...
awaiting_callback(
    ...
    case props:get_value(CallId, Candidates) of
        ...
        CtrlQ ->
            ...
            acdc_agent_stats:agent_connected(
                AccountId, AgentId, OriginalMemberCallId, CIDName, CIDNumber
            ),
            _ = acdc_stats:call_handled(AccountId, QueueId, OriginalMemberCallId, AgentId),
...
awaiting_callback(
    'cast',
    {'leg_created', CallId, OtherLegCallId} = Evt,
    #state{
        agent_listener = AgentListener,
        member_callback_candidates = Candidates
    } = State
    ...
    case props:get_value(CallId, Candidates) of
        ...
        CtrlQ ->
            ...
            {'next_state', 'awaiting_callback', State#state{
                member_callback_candidates = Candidates1
            }}
    end;
...
awaiting_callback(
    'cast',
    ?DESTROYED_CHANNEL(OriginalMemberCallId, _Cause),
    #state{
        member_original_call_id = OriginalMemberCallId,
        monitoring = 'true'
    } = State
...
answered(
    ...
    _ = acdc_stats:call_missed(
        AccountId, QueueId, AgentId, original_call_id(State), <<"dialplan_error">>
    ),
...
answered(
    'cast',
    {'channel_transferee', Transferor, Transferee},
    #state{
        account_id = AccountId,
        agent_id = AgentId,
    ...
    {'next_state', 'outbound',
        start_outbound_call_handling(Transferee, clear_call(State, 'ready'))};
...
answered(
    'cast',
    ?DESTROYED_CHANNEL(CallId, Cause),
    #state{
        member_call_id = CallId,
        outbound_call_ids = []
    } = State
...
answered(
    'cast',
    ?NEW_CHANNEL_FROM(CallId, _, _, MemberCallId),
    #state{member_call_id = MemberCallId} = State
) ->
...
wrapup(
    ...
    lager:debug("recv sync_req from ~s", [kz_json:get_value(<<"Process-ID">>, JObj)]),
    acdc_agent_listener:send_sync_resp(AgentListener, 'wrapup', JObj, [
        {<<"Time-Left">>, time_left(Ref)}
    ]),
    {'next_state', 'wrapup', State};
...
wrapup(
    ...
    {'next_state', 'wrapup', State, {'reply', From, current_call(Call, 'wrapup', QueueId, Start)}};
wrapup(
    'info',
    {'timeout', Ref, ?WRAPUP_FINISHED},
    #state{
        wrapup_ref = Ref,
        agent_listener = AgentListener
    } = State
...
paused(
    ...
    lager:debug("recv sync_req from ~s", [kz_json:get_value(<<"Process-ID">>, JObj)]),
    acdc_agent_listener:send_sync_resp(AgentListener, 'paused', JObj, [
        {<<"Time-Left">>, time_left(Ref)}
    ]),
    {'next_state', 'paused', State};
...
outbound(
    ...
    acdc_util:bind_to_call_events(CallId, AgentListener),
    {'next_state', 'outbound', State#state{
        outbound_call_ids = [CallId | lists:delete(CallId, OutboundCallIds)]
    }};
outbound(
    'cast',
...
inbound(
    ...
    acdc_util:bind_to_call_events(CallId, AgentListener),
    {'next_state', 'inbound', State#state{
        inbound_call_ids = [CallId | lists:delete(CallId, InboundCallIds)]
    }};
inbound(
    'cast',
    ?NEW_CHANNEL_TO(CallId, _, _),
...
inbound({call, From}, 'current_call', State) ->
    {'next_state', 'inbound', State, {'reply', From, 'undefined'}};
...
handle_event(
    ...
    _ = acdc_stats:call_missed(
        AccountId, QueueId, AgentId, original_call_id(State), <<"agent pausing">>
    ),
    NewServerRefState = clear_call(State, 'failed'),
...
handle_event(
    {'add_acdc_queue', QueueId}, StateName, #state{agent_listener = AgentListener} = State
) ->
...
handle_event({'update_presence', PresenceId, PresenceState}, 'ready', State) ->
    ...
    {'next_state', 'ready', State};
handle_event(
    {'update_presence', _, _} = Event, StateName, #state{agent_state_updates = Queue} = State
) ->
    NewQueue = [Event | Queue],
    {'next_state', StateName, State#state{agent_state_updates = NewQueue}};
handle_event({'refresh', AgentJObj}, StateName, #state{agent_listener = AgentListener} = State) ->
    acdc_agent_listener:refresh_config(
        AgentListener,
        AgentJObj,
        StateName
    ),
    ...
    case kz_json:get_value(<<"owner_id">>, EP) of
        AgentId ->
            ...
            {'next_state', StateName,
                State#state{endpoints = maybe_add_endpoint(EPId, EP, EPs, AccountId)}, 'hibernate'};
        _OwnerId ->
            lager:debug("device ~s edited, owner now ~s, maybe removing it", [EPId, _OwnerId]),
            {'next_state', StateName,
                State#state{endpoints = maybe_remove_endpoint(EPId, EPs, AccountId)}, 'hibernate'}
    end;
handle_info(
    {'endpoint_deleted', EP},
    StateName,
    ...
    {'next_state', StateName, State#state{endpoints = maybe_remove_endpoint(EPId, EPs, AccountId)},
        'hibernate'};
handle_info(
    {'endpoint_created', EP},
    StateName,
        ...
        _OwnerId ->
            ...
            case kz_json:get_value([<<"hotdesk">>, <<"users">>, AgentId], EP) of
                ...
                _ ->
                    ...
                    {'next_state', StateName,
                        State#state{endpoints = maybe_add_endpoint(EPId, EP, EPs, AccountId)},
                        'hibernate'}
            end
    end;
...
) when MemberCallId =/= MemberCallId1 ->
    lager:debug(
        "cancelling ~s (failed originate from queue call ~s",
        [CallId, MemberCallId]
    ),
    acdc_agent_listener:channel_hungup(AgentListener, CallId),
    {'next_state', StateName, State};
...
% send immediately
start_wrapup_timer(Timeout) when Timeout =< 0 -> start_wrapup_timer(1);
...
clear_call(
    ...
    'failed'
) when is_integer(Max), (Max - Fails) =< 1 ->
    agent_logout(self()),
...
-spec current_call(
    kapps_call:call() | 'undefined', atom(), kz_term:ne_binary(), 'undefined' | kz_term:kz_now()
) ->
    kz_term:api_object().
current_call('undefined', _, _, _) ->
    'undefined';
...
-spec start_inbound_call_handling(
    kz_term:ne_binary() | kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), state()
) -> state().
start_inbound_call_handling(
    CallId,
    Number,
    Name,
    #state{
    ...
    case time_left(WRef) of
        ...
        _W ->
            case time_left(PRef) of
                ...
                'infinity' ->
                    apply_state_updates(clear_call(State, 'paused'));
                _P ->
...
missed_reason(<<"ALLOTTED_TIMEOUT">>) ->
    <<"timeout">>;
missed_reason(<<"NO_USER_RESPONSE">>) ->
    <<"rejected">>;
missed_reason(<<"CALL_REJECTED">>) ->
    <<"rejected">>;
missed_reason(<<"USER_BUSY">>) ->
    <<"rejected">>;
...
-spec maybe_add_endpoint(
    kz_term:ne_binary(), kz_json:object(), kz_json:objects(), kz_term:ne_binary()
) -> any().
maybe_add_endpoint(EPId, EP, EPs, AccountId) ->
    case lists:partition(fun(E) -> find_endpoint_id(E) =:= EPId end, EPs) of
        {[], _} ->
            ...
            [
                begin
                    monitor_endpoint(convert_to_endpoint(EP), AccountId),
                    EP
                end
                | EPs
            ];
...
-spec maybe_remove_endpoint(kz_term:ne_binary(), kz_json:objects(), kz_term:ne_binary()) ->
    kz_json:objects().
...
convert_to_endpoint(EPDoc) ->
    Setters = [
        {fun kapps_call:set_account_id/2, kz_doc:account_id(EPDoc)},
...
-spec get_endpoints(
    kz_json:objects(), kapps_call:call(), kz_term:api_binary(), kz_term:api_binary()
) ->
    {'ok', kz_json:objects()}
    | {'error', any()}.
...
changed_endpoints(OrigEPs, EPs) -> changed_endpoints(OrigEPs, EPs, []).

changed_endpoints([], [], Add) ->
    {Add, []};
changed_endpoints(OrigEPs, [], Add) ->
    {Add, OrigEPs};
...
notify_method(Url, 'get', Data) ->
    notify(
        uri(Url, kz_http_util:json_to_querystring(Data)),
        [],
        'get',
        <<>>,
        []
...
-spec notify(kz_term:ne_binary(), kz_term:proplist(), 'get' | 'post', binary(), kz_term:proplist()) ->
    'ok'.
...
uri(URI, QueryString) ->
    ...
    case kz_http_util:urlsplit(URI) of
        ...
        {Scheme, Host, Path, QS, Fragment} ->
            kz_http_util:urlunsplit(
                {Scheme, Host, Path, <<QS/binary, "&", QueryBinary/binary>>, Fragment}
            )
    end.
...
apply_state_updates(
    ...
    apply_state_updates_fold(
        {'next_state', FoldDefaultState, State#state{agent_state_updates = []}}, lists:reverse(Q)
    ).

-spec apply_state_updates_fold({'next_state', atom(), state()}, list()) ->
    kz_term:handle_fsm_ret(state()).
apply_state_updates_fold(
    {Next, StateName,
        #state{
            account_id = AccountId,
    ...
    case not Monitor andalso StateName of
        ...
        'wrapup' ->
            acdc_agent_stats:agent_wrapup(AccountId, AgentId, time_left(WRef));
        'paused' ->
...
apply_state_updates_fold({_, _, State}, [{'agent_logout'} | _]) ->
    ...
    handle_agent_logout(State);
apply_state_updates_fold({_, _, State} = Acc, [
    {'update_presence', PresenceId, PresenceState} | Updates
]) ->
    handle_presence_update(PresenceId, PresenceState, State),
