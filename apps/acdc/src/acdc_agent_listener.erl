...
-define(BINDINGS(AccountId, AgentId), [
    {'self', []},
    {'acdc_agent', [
...
-define(RESPONDERS, [
    {{'acdc_agent_handler', 'handle_sync_req'}, [{<<"agent">>, <<"sync_req">>}]},
    {{'acdc_agent_handler', 'handle_sync_resp'}, [{<<"agent">>, <<"sync_resp">>}]},
    {{'acdc_agent_handler', 'handle_stats_req'}, [{<<"agent">>, <<"stats_req">>}]},
    {{'acdc_agent_handler', 'handle_call_event'}, [{<<"call_event">>, <<"*">>}]},
    {{'acdc_agent_handler', 'handle_originate_resp'}, [{<<"resource">>, <<"*">>}]},
    {{'acdc_agent_handler', 'handle_call_event'}, [{<<"error">>, <<"*">>}]},
    {{'acdc_agent_handler', 'handle_member_message'}, [{<<"member">>, <<"*">>}]},
    {{'acdc_agent_handler', 'handle_agent_message'}, [{<<"agent">>, <<"*">>}]},
...
-spec originate_callback_to_agent(
    pid(),
    kapps_call:call(),
    kz_json:object(),
    kz_json:objects(),
...
originate_callback_to_agent(Srv, Call, WinJObj, EPs, CDRUrl, RecordingUrl, Number) ->
    gen_listener:cast(
        Srv, {'originate_callback_to_agent', Call, WinJObj, EPs, CDRUrl, RecordingUrl, Number}
    ).
...
maybe_update_presence_id(Srv, Id) -> gen_listener:cast(Srv, {'presence_id', Id}).

-spec presence_update(pid(), api_kz_term:ne_binary()) -> 'ok'.
presence_update(_, 'undefined') -> 'ok';
presence_update(Srv, PresenceState) -> gen_listener:cast(Srv, {'presence_update', PresenceState}).
...
handle_cast(
    {'refresh_config', JObj, StateName},
    #state{
        agent_priority = Priority0,
        skills = Skills0,
        agent_queues = Queues
    } = State
) ->
    ...
    {Add, Rm} =
        case is_prio_or_skills_updated(Priority0, Skills0, Priority, Skills) of
            'true' ->
                ...
                {Add1, Rm1};
            'false' ->
                acdc_agent_util:changed(Queues, Qs)
        end,
...
handle_cast({'add_acdc_queue', Q, StateName}, #state{agent_queues = Qs} = State) when
    is_binary(Q)
->
        ...
        _ ->
            case props:get_value(CallId, ACallIds) of
                'true' ->
                    ...
                    acdc_util:unbind_from_call_events(CallId),
                    {'noreply', State#state{agent_call_ids = lists:delete(CallId, ACallIds)},
                        'hibernate'};
                'undefined' ->
    ...
    case catch kapps_call:call_id(Call) of
        CallId ->
            ...
            {'noreply',
                ...
                'hibernate'};
        _MCallId ->
            lager:debug("retry call id(~s) is not our member call id ~p, ignoring", [
                CallId, _MCallId
            ]),
            {'noreply', State}
    end;
handle_cast(
    {'member_connect_retry', WinJObj},
    #state{
        my_id = MyId,
        agent_id = AgentId
    } = State
    ...
    {'noreply', State};
handle_cast(
    {'bridge_to_member', Call, WinJObj, EPs, CDRUrl, RecordingUrl},
    #state{
        is_thief = 'false',
        agent_queues = Qs,
        acct_id = AccountId,
    ...
    ShouldRecord = should_record_endpoints(
        EPs,
        record_calls(Agent),
        kz_json:is_true(<<"Record-Caller">>, WinJObj, 'false')
    ),

    AgentCallIds = lists:append(
        maybe_connect_to_agent(MyQ, EPs, Call, RingTimeout, AgentId, CDRUrl),
        ACallIds
    ),

    gen_listener:add_binding(self(), 'acdc_agent', [
        {'callid', call_id(Call)},
    ...
    {'noreply',
        ...
        'hibernate'};
handle_cast(
    {'bridge_to_member', Call, WinJObj, _, CDRUrl, RecordingUrl},
    #state{
        is_thief = 'true',
        agent = Agent,
        agent_id = AgentId,
...
handle_cast({'monitor_call', Call, WinJObj, RecordingUrl}, State) ->
    ...
    {'noreply',
        ...
        'hibernate'};
handle_cast(
    {'originate_callback_to_agent', Call, WinJObj, EPs, CDRUrl, RecordingUrl, Number},
    #state{
        agent_queues = Qs,
        acct_id = AccountId,
        agent_id = AgentId,
    ...
    AgentCallIds = lists:append(
        maybe_originate_callback(MyQ, EPs, Call, RingTimeout, AgentId, CDRUrl, Number),
        ACallIds
    ),
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'member_connect_accepted', ACallId},
    #state{
        msg_queue_id = AmqpQueue,
        call = Call,
        acct_id = AccountId,
...
handle_cast(
    {'member_connect_accepted', ACallId, NewCall},
    #state{
        msg_queue_id = AmqpQueue,
        call = Call,
        acct_id = AccountId,
    ...
    send_member_connect_accepted(
        AmqpQueue, call_id(Call), call_id(NewCall), AccountId, AgentId, MyId
    ),
    _ = [send_agent_busy(AccountId, AgentId, QueueId, Call) || QueueId <- Qs],
    {'noreply',
        State#state{
...
handle_cast(
    {'member_callback_accepted', ACall},
    #state{
        msg_queue_id = AmqpQueue,
        call = Call,
        acct_id = AccountId,
...
handle_cast(
    {'outbound_call', CallId, Number, Name},
    #state{
        agent_id = AgentId,
        acct_id = AccountId,
        agent_queues = Qs
    ...
    _ = [
        send_agent_busy(AccountId, AgentId, QueueId, Call, 'outbound', Number, Name)
     || QueueId <- Qs
    ],
...
handle_cast(
    {'inbound_call', CallId, Number, Name},
    #state{
        agent_id = AgentId,
        acct_id = AccountId,
        agent_queues = Qs
    ...
    Call = kapps_call:set_call_id(CallId, kapps_call:new()),
    _ = [
        send_agent_busy(AccountId, AgentId, QueueId, Call, 'inbound', Number, Name)
     || QueueId <- Qs
    ],
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'send_sync_resp', Status, ReqJObj, Options},
    #state{
        my_id = MyId,
        acct_id = AccountId,
        agent_id = AgentId,
...
handle_event(_JObj, #state{fsm_pid = 'undefined'}) ->
    'ignore';
...
send_member_connect_retry(JObj, MyId, AgentId) ->
    send_member_connect_retry(
        kz_json:get_value(<<"Server-ID">>, JObj),
        call_id(JObj),
        MyId,
        AgentId
    ).

-spec send_member_connect_retry(
...
-spec send_member_connect_accepted(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
-spec send_member_callback_accepted(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
call_id(Call) ->
    case kapps_call:is_call(Call) of
        ...
        'false' ->
            ...
            lists:foldl(
                fun
                    (K, 'undefined') -> kz_json:get_value(K, Call);
                    (_, CallId) -> CallId
                end,
                'undefined',
                Keys
...
maybe_connect_to_agent(MyQ, EPs, Call, Timeout, AgentId, _CdrUrl) ->
    ...
    {ACallIds, Endpoints} = lists:foldl(
        fun(EP, {Cs, Es}) ->
...
-spec outbound_call_id(kapps_call:call() | kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
send_availability_update(QueueId, 'ready', #state{
    agent_id = AgentId,
    agent_priority = Priority,
    skills = Skills,
    acct_id = AccountId
...
send_availability_update(QueueId, _, #state{
    agent_id = AgentId,
    acct_id = AccountId
}) ->
    send_agent_busy(AccountId, AgentId, QueueId).

-spec send_agent_available(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    agent_priority(),
...
update_my_queues_of_change(AccountId, AgentId, Call, Qs) ->
    ...
    _ = [
        kapi_acdc_queue:publish_agent_change([{<<"Queue-ID">>, QueueId} | Props])
     || QueueId <- Qs
    ],
    'ok'.

-spec is_prio_or_skills_updated(
    agent_priority(), kz_term:ne_binaries(), agent_priority(), kz_term:ne_binaries()
) ->
    boolean().
is_prio_or_skills_updated(Priority0, Skills0, Priority, Skills) ->
    Priority0 =/= Priority orelse
        Skills0 =/= Skills.

-spec should_record_endpoints(kz_json:objects(), boolean(), kz_term:api_boolean()) -> boolean().
should_record_endpoints(_EPs, 'true', _) ->
    'true';
should_record_endpoints(_EPs, 'false', 'true') ->
    'true';
...
maybe_start_recording(Call, 'true', Url) ->
    ...
    case acdc_recordings_sup:new(Call, RecordingJObj) of
        ...
        _E ->
            lager:debug("failed to start recording: ~p", [_E])
    end.
...
record_calls(Agent) ->
    not is_thief(Agent) andalso
        kz_json:is_true(<<"record_calls">>, Agent, 'false').
...
stop_agent_leg('undefined', _) ->
    lager:debug("agent call id not defined");
stop_agent_leg(_, 'undefined') ->
    lager:debug("agent ctrl queue not defined");
...
filter_agent_calls(ACallIds, ACallId) ->
    ...
    lists:filter(
        fun
            ({ACancelId, 'undefined'}) when ACancelId =/= ACallId ->
                lager:debug(
                    "~s will have to be cancelled when ctrl queue arrives",
                    [ACancelId]
                ),
                'true';
