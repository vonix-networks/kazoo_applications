...
-export([
    update_status/3, update_status/4,
    ...
    case most_recent_ets_status(AccountId, AgentId) of
        ...
        {'error', _ErrJObj} ->
            case kz_json:is_valid_json_object(_ErrJObj) of
                true ->
                    lager:debug("failed to get ETS stats: ~p", [
                        kz_json:get_value(<<"Error-Reason">>, _ErrJObj)
                    ]);
                false ->
                    lager:debug("failed to get ETS stats: ~p", [_ErrJObj])
            end,
...
most_recent_db_status(AccountId, AgentId) ->
    Opts = [
        {'startkey', [AgentId, kz_time:now_s()]},
        {'limit', 1},
        'descending'
    ],
    case
        kz_datamgr:get_results(
            acdc_stats_util:db_name(AccountId), <<"agent_stats/status_log">>, Opts
        )
    of
        {'ok', [StatusJObj]} ->
            {'ok', kz_json:get_value(<<"value">>, StatusJObj)};
        {'ok', []} ->
            lager:debug("could not find a recent status for agent ~s, checking previous modb", [
                AgentId
            ]),
...
most_recent_statuses(AccountId, AgentId, Options) ->
    ETS = kz_util:spawn_monitor(fun async_most_recent_ets_statuses/4, [
        AccountId, AgentId, Options, self()
    ]),
    DB = maybe_start_db_lookup(
        'async_most_recent_db_statuses',
...
-spec maybe_start_db_lookup(
    atom(), fun(), kz_term:ne_binary(), kz_term:api_binary(), list(), pid()
) ->
    kz_term:pid_ref() | 'undefined'.
maybe_start_db_lookup(F, Fun, AccountId, AgentId, Options, Self) ->
    case kz_cache:fetch_local(?CACHE_NAME, db_fetch_key(F, AccountId, AgentId, Options)) of
        {'ok', _} -> {'undefined', db_fetch_key(F, AccountId, AgentId, Options)};
        {'error', 'not_found'} -> kz_util:spawn_monitor(Fun, [AccountId, AgentId, Options, Self])
    end.

db_fetch_key(F, AccountId, AgentId, Options) -> {F, AccountId, AgentId, Options}.

-type receive_info() :: [{pid(), reference()} | {'undefined', reference()}].
...
-spec receive_statuses(receive_info(), kz_json:object()) ->
    kz_json:object().
receive_statuses([], AccJObj) ->
    AccJObj;
receive_statuses([{'undefined', Ref} | Reqs], AccJObj) ->
    {'ok', Statuses} = kz_cache:fetch_local(?CACHE_NAME, Ref),
...
-spec async_most_recent_ets_statuses(
    kz_term:ne_binary(), kz_term:api_binary(), kz_term:proplist(), pid()
) -> 'ok'.
...
-spec async_most_recent_db_statuses(
    kz_term:ne_binary(), kz_term:api_binary(), kz_term:proplist(), pid()
) -> 'ok'.
    ...
    case most_recent_db_statuses(AccountId, AgentId, Options) of
        {'ok', Statuses} ->
            ...
            kz_cache:store_local(
                ?CACHE_NAME,
                db_fetch_key('async_most_recent_db_statuses', AccountId, AgentId, Options),
                Statuses
            ),
            'ok';
        {'error', _E} ->
...
most_recent_ets_statuses(AccountId, AgentId, Options) ->
    ...
    of
        {'error', _} = E -> E;
        {'ok', Resp} -> {'ok', kz_json:get_value([<<"Agents">>], Resp, kz_json:new())}
    end.
...
build_timestamp_view_options([], ViewOptions) ->
    ViewOptions;
...
build_agent_view_options(_AgentId, [], ViewOptions) ->
    ViewOptions;
...
-spec find_most_recent_fold(integer() | kz_term:ne_binary(), kz_json:object(), {
...
cleanup_db_statuses(Stats, ReqOpts) ->
    Filters = [
        case props:get_value(<<"Status">>, ReqOpts) of
    ...
    lists:foldl(
        fun(S, Acc) ->
            kz_json:set_value(
                [kz_json:get_binary_value(Key1, S), kz_json:get_binary_value(Key2, S)],
