...
handle_route_req(JObj, Props) ->
    ...
    Call = kapps_call:set_controller_queue(
...
maybe_route_respond(_JObj, _Call, 'undefined', _Id) ->
    'ok';
maybe_route_respond(ReqJObj, Call, AccountId, Id) ->
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), Id) of
        {'error', _} -> 'ok';
        {'ok', Doc} -> maybe_route_respond(ReqJObj, Call, AccountId, Id, kz_doc:type(Doc))
    end.
...
maybe_route_respond(_ReqJObj, _Call, _AccountId, _Id, _) ->
    'ok'.
...
send_route_response(ReqJObj, Call, AccountId, Id, Type) ->
    ...
    Resp = [
        {?KEY_MSG_ID, kz_api:msg_id(ReqJObj)},
...
update_acdc_actor(_Call, 'undefined', _) ->
    'ok';
update_acdc_actor(_Call, _, 'undefined') ->
    'ok';
...
update_acdc_actor(Call, AgentId, <<"user">>) ->
    ...
    case acdc_agent_util:most_recent_status(AccountId, AgentId) of
        {'ok', <<"logout">>} ->
            update_acdc_agent(
                Call, AccountId, AgentId, <<"login">>, fun kapi_acdc_agent:publish_login/1
            );
        {'ok', <<"pause">>} ->
            update_acdc_agent(
                Call, AccountId, AgentId, <<"resume">>, fun kapi_acdc_agent:publish_resume/1
            );
        {'ok', _S} ->
            update_acdc_agent(
                Call, AccountId, AgentId, <<"logout">>, fun kapi_acdc_agent:publish_logout/1
            )
    end.

-spec update_acdc_agent(
    kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), fun()
) -> kz_term:ne_binary().
...
update_agent_device(Call, AgentId, <<"login">>) ->
    lager:debug("need to set owner_id to ~s on device ~s", [
        AgentId, kapps_call:authorizing_id(Call)
    ]),
    {'ok', Device} = kz_datamgr:open_doc(
        kapps_call:account_db(Call), kapps_call:authorizing_id(Call)
    ),
    lager:debug("setting owner_id from ~s to ~s", [
        kz_json:get_value(<<"owner_id">>, Device), AgentId
    ]),
    ...
    case kz_json:get_value(<<"owner_id">>, Device) of
        AgentId ->
            ...
            {'ok', _} = kz_datamgr:save_doc(
                kapps_call:account_db(Call), kz_json:delete_key(<<"owner_id">>, Device)
            );
        _Other ->
...
move_agent_device(Call, AgentId, Device) ->
    ...
    {'ok', _} = kz_datamgr:save_doc(
        kapps_call:account_db(Call), kz_json:set_value(<<"owner_id">>, AgentId, Device)
    ).

-spec send_new_status(kz_term:ne_binary(), kz_term:ne_binary(), kz_amqp_worker:publish_fun()) ->
    'ok'.
send_new_status(AccountId, AgentId, PubFun) ->
