...
handle_call_event(JObj, Props) ->
    case kz_util:get_event_type(JObj) of
        ...
        {<<"error">>, Name} ->
            lager:error("Call event error: ~s ~s", [
                Name, kz_json:get_value(<<"Error-Message">>, JObj)
            ]),
            ok
    end.
...
-spec handle_member_call(kz_json:object(), kz_term:proplist(), gen_listener:basic_deliver()) ->
    'ok'.
...
handle_queue_change(AccountDb, AccountId, QueueId, ?DOC_EDITED) ->
    ...
    case acdc_queues_sup:find_queue_supervisor(AccountId, QueueId) of
        ...
        QueueSup when is_pid(QueueSup) ->
            ...
            Refresher = fun(Sup) ->
                acdc_queue_fsm:refresh(acdc_queue_worker_sup:fsm(Sup), JObj)
            end,
...
handle_queue_change(_, AccountId, QueueId, ?DOC_DELETED) ->
    ...
    case acdc_queues_sup:find_queue_supervisor(AccountId, QueueId) of
        'undefined' ->
            lager:debug("no queue(~s) started for account ~s", [QueueId, AccountId]);
        P when is_pid(P) ->
...
update_probe(_JObj, 'undefined', _, _) ->
    'ok';
