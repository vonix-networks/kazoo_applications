...
-define(RESPONDERS, [
    {{'acdc_queue_handler', 'handle_call_event'}, [{<<"call_event">>, <<"*">>}]},
    {{'acdc_queue_handler', 'handle_call_event'}, [{<<"error">>, <<"*">>}]},
    {{'acdc_queue_handler', 'handle_member_call_cancel'}, [{<<"member">>, <<"call_cancel">>}]},
    ...
    {{'acdc_queue_handler', 'handle_sync_req'}, [{<<"queue">>, <<"sync_req">>}]},
    {{'acdc_queue_handler', 'handle_member_callback_reg'}, [{<<"member">>, <<"callback_reg">>}]},
...
handle_cast(
    {'member_call', MemberCallJObj, Delivery},
    #state{
        queue_id = QueueId,
        account_id = AccountId
    } = State
    ...
    gen_listener:add_binding(self(), 'acdc_queue', [
        {'restrict_to', ['member_call_result']},
        {'account_id', AccountId},
        {'queue_id', QueueId},
        {'callid', CallId}
    ]),
...
handle_cast(
    {'member_connect_req', MemberCallJObj, Delivery, _Url},
    #state{
        my_q = MyQ,
        my_id = MyId,
        account_id = AccountId,
    ...
    gen_listener:add_binding(self(), 'acdc_queue', [
        {'restrict_to', ['member_callback_reg', 'member_call_result']},
        {'account_id', AccountId},
        {'queue_id', QueueId},
        {'callid', CallId}
    ]),
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'member_connect_win', RespJObj, QueueOpts},
    #state{
        my_q = MyQ,
        my_id = MyId,
        call = Call,
    ...
    {'noreply',
        ...
        'hibernate'};
handle_cast(
    {'member_connect_satisfied', RespJObj, QueueOpts},
    #state{
        my_q = MyQ,
        my_id = MyId,
        call = Call,
...
handle_cast(
    ...
    send_member_call_failure(
        ...
        MyId,
        AgentId,
        <<"call timed out waiting in queue">>
    ),
...
handle_cast(
    ...
    send_member_call_failure(
        ...
        MyId,
        AgentId,
        <<"Caller exited the queue via DTMF">>
    ),

    {'noreply', clear_call_state(State), 'hibernate'};
handle_cast(
    {'exit_member_call_empty'},
    #state{
        delivery = Delivery,
        call = Call,
        shared_pid = Pid,
...
handle_cast(
    ...
    {'noreply', clear_call_state(State), 'hibernate'};
handle_cast(
    {'finish_member_call', _AcceptJObj},
    #state{
        delivery = Delivery,
        call = Call,
        shared_pid = Pid,
...
handle_cast({'cancel_member_call', _RejectJObj}, #state{delivery = 'undefined'} = State) ->
    ...
    {'noreply', State};
handle_cast(
    {'cancel_member_call', _RejectJObj},
    #state{
        queue_id = QueueId,
        account_id = AccountId,
        delivery = Delivery,
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_timeout_agent(
    kz_term:api_object(), kz_term:ne_binary(), kapps_call:call(), kz_term:api_object()
) -> 'ok'.
maybe_timeout_agent('undefined', _QueueId, _Call, _JObj) ->
    'ok';
maybe_timeout_agent(_AgentId, _QueueId, _Call, 'undefined') ->
    'ok';
maybe_timeout_agent(_AgentId, QueueId, Call, JObj) ->
    lager:warning(
        "timing out winning agent ~s because they should not be able to pick up after the queue ~s timeout",
        [_AgentId, QueueId]
    ),
    send_agent_timeout(JObj, Call, QueueId).

-spec send_member_connect_req(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
-spec send_member_connect_satisfied(
    kz_json:object(),
    kapps_call:call(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
send_agent_timeout(RespJObj, Call, QueueId) ->
    ...
    publish(
        kz_json:get_value(<<"Server-ID">>, RespJObj),
        Prop,
...
publish_sync_resp(Strategy, StrategyState, ReqJObj, Id) ->
    ...
    publish(
        kz_json:get_value(<<"Server-ID">>, ReqJObj), Resp, fun kapi_acdc_queue:publish_sync_resp/2
    ).
...
clear_call_state(
    ...
    case Call of
        ...
        _ ->
            CallId = kapps_call:call_id(Call),
            gen_listener:rm_binding(self(), 'acdc_queue', [
                {'restrict_to', ['member_callback_reg', 'member_call_result']},
                {'account_id', AccountId},
                {'queue_id', QueueId},
                {'callid', CallId}
...
-spec publish(kz_term:ne_binary(), kz_term:api_terms(), fun(
    (kz_term:ne_binary(), kz_term:api_terms()) -> 'ok'
)) -> 'ok'.
publish(Q, Req, F) ->
    try F(Q, Req)
    catch
