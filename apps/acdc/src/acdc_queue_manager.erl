...
-define(RESPONDERS, [
    ...
    {{'acdc_queue_handler', 'handle_stats_req'}, [{<<"queue">>, <<"stats_req">>}]},
    {{'acdc_queue_handler', 'handle_presence_probe'}, [{<<"presence">>, <<"probe">>}]},
    {{?MODULE, 'handle_member_call'}, [{<<"member">>, <<"call">>}]},
    {{?MODULE, 'handle_member_call_success'}, [{<<"member">>, <<"call_success">>}]},
    {{?MODULE, 'handle_member_call_cancel'}, [{<<"member">>, <<"call_cancel">>}]},
    {{?MODULE, 'handle_agent_change'}, [{<<"queue">>, <<"agent_change">>}]},
    {{?MODULE, 'handle_agents_available_req'}, [{<<"queue">>, <<"agents_available_req">>}]},
    {{?MODULE, 'handle_queue_member_add'}, [{<<"queue">>, <<"member_add">>}]},
    {{?MODULE, 'handle_queue_member_remove'}, [{<<"queue">>, <<"member_remove">>}]},
    {{?MODULE, 'handle_member_callback_reg'}, [{<<"member">>, <<"callback_reg">>}]}
]).
...
-define(SECONDARY_QUEUE_OPTIONS(MaxPriority), [
    {'exclusive', 'false'},
...
handle_member_call_success(JObj, Prop) ->
    gen_listener:cast(
        props:get_value('server', Prop),
        {'handle_queue_member_remove', kz_json:get_value(<<"Call-ID">>, JObj)}
    ).
...
-spec should_ignore_member_call(kz_term:server_ref(), kapps_call:call(), kz_json:object()) ->
    boolean().
...
-spec should_ignore_member_call(
    kz_term:server_ref(), kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary()
) -> boolean().
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_call(any(), kz_term:pid_ref(), mgr_state()) ->
    kz_term:handle_call_ret_state(mgr_state()).
handle_call(
    {'should_ignore_member_call', {AccountId, QueueId, CallId} = K},
    _,
    #state{
    ...
    case catch dict:fetch(K, Dict) of
        {'EXIT', _} ->
            case queue_member(CallId, Calls) of
                'undefined' ->
                    lager:error(
                        "call ~p not in list of current_member_calls or ignored_member_calls)", [
                            CallId
                        ]
                    ),
                    {'reply', 'true', State};
...
handle_call({'up_next', _CallId}, _, #state{strategy = 'all'} = State) ->
    {'reply', 'true', State};
handle_call(
    {'up_next', CallId},
    _,
...
handle_call(
    ...
    case pqueue4:pout(Agents) of
        {{'value', Winner, Priority}, Agents1} ->
            {'reply', Winner,
                State#state{
                    strategy_state = SS#strategy_state{
                        agents = pqueue4:in(Winner, Priority, Agents1)
                    }
                },
                'hibernate'};
        {'empty', _} ->
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'agent_available', AgentId, Priority, Skills},
    #state{
        supervisor = QueueSup,
        strategy = Strategy
    } = State
) when is_binary(AgentId) ->
    StrategyState1 = update_strategy_with_agent(
        State, AgentId, Priority, Skills, 'add', 'undefined'
    ),
...
handle_cast({'agent_available', JObj}, State) ->
    handle_cast(
        {'agent_available', kz_json:get_ne_binary_value(<<"Agent-ID">>, JObj),
            kz_json:get_integer_value(<<"Priority">>, JObj, 0),
            kz_json:get_list_value(<<"Skills">>, JObj, [])},
        State
    );
handle_cast({'agent_ringing', AgentId, Priority}, #state{strategy = Strategy} = State) when
    is_binary(AgentId)
->
...
handle_cast({'agent_ringing', JObj}, State) ->
    handle_cast(
        {'agent_ringing', kz_json:get_ne_binary_value(<<"Agent-ID">>, JObj),
            kz_json:get_integer_value(<<"Priority">>, JObj, 0)},
        State
    );
handle_cast({'agent_busy', AgentId, Priority}, #state{strategy = Strategy} = State) when
    is_binary(AgentId)
->
...
handle_cast({'agent_busy', JObj}, State) ->
    handle_cast(
        {'agent_busy', kz_json:get_ne_binary_value(<<"Agent-ID">>, JObj),
            kz_json:get_integer_value(<<"Priority">>, JObj, 0)},
        State
    );
handle_cast({'agent_unavailable', AgentId, Priority}, #state{strategy = Strategy} = State) when
    is_binary(AgentId)
->
    lager:info("agent ~s unavailable, maybe updating strategy ~s", [AgentId, Strategy]),

    StrategyState1 = update_strategy_with_agent(
        State, AgentId, Priority, [], 'remove', 'undefined'
    ),
    {'noreply', State#state{strategy_state = StrategyState1}, 'hibernate'};
handle_cast({'agent_unavailable', JObj}, State) ->
    handle_cast(
        {'agent_unavailable', kz_json:get_ne_binary_value(<<"Agent-ID">>, JObj),
            kz_json:get_integer_value(<<"Priority">>, JObj, 0)},
        State
    );
handle_cast(
...
handle_cast(
    ...
    Call1 = kapps_call:exec(
        [
            {fun kapps_call:set_custom_channel_var/3, <<"Queue-Position">>, Position},
    ...
    StatSkills =
        case Strategy of
            ...
            _ when Skills /= [] ->
                lager:warning(
                    "skills ~p required, but queue ~s is not set to skills_based_round_robin", [
                        Skills, QueueId
                    ]
                ),
                'undefined';
    ...
    State1 = lists:foldl(
        fun({Updater, Args}, StateAcc) -> apply(Updater, Args ++ [StateAcc]) end,
        State,
        [
...
handle_cast(
    ...
    {'noreply', State1};
handle_cast(
    {'handle_queue_member_remove', CallId},
    #state{
        current_member_calls = Calls,
    ...
    {'noreply', State2};
handle_cast(
    {'handle_member_callback_reg', JObj},
    #state{
        account_id = AccountId,
        queue_id = QueueId,
        current_member_calls = Calls,
...
handle_event(_JObj, #state{
    enter_when_empty = EnterWhenEmpty,
    moh = MOH
}) ->
    {'reply', [
        {'enter_when_empty', EnterWhenEmpty},
...
-spec queue_member_insert_position(kz_term:ne_binary(), kz_term:api_integer(), list()) ->
    pos_integer().
...
-spec queue_member_insert_position(
    kz_term:ne_binary(), kz_term:api_integer(), list(), pos_integer()
) -> pos_integer().
...
-spec add_queue_member(
    kapps_call:call(), kz_term:api_non_neg_integer(), pos_integer(), mgr_state()
) -> mgr_state().
...
add_queue_member(Call, Priority, Position, State) ->
    ...
    lager:info("add member call ~s (~p) at position ~p", [
        kapps_call:call_id(Call), acdc_util:caller_id(Call), Position
    ]),
...
-spec maybe_remove_ignored_member_call(kz_term:ne_binary() | kapps_call:call(), mgr_state()) ->
    mgr_state().
maybe_remove_ignored_member_call(
    CallId,
    #state{
        account_id = AccountId,
        queue_id = QueueId,
    ...
    case catch dict:fetch(K, Dict) of
        {'EXIT', _} -> State;
        _Res -> State#state{ignored_member_calls = dict:erase(K, Dict)}
    end;
maybe_remove_ignored_member_call(Call, State) ->
    maybe_remove_ignored_member_call(kapps_call:call_id(Call), State).

-spec publish_agents_available_resp(
    kz_term:ne_binary(), kz_term:ne_binary(), non_neg_integer(), kz_json:object()
) -> 'ok'.
publish_agents_available_resp(AccountId, QueueId, AgentCount, JObj) ->
    Resp = [
        {<<"Account-ID">>, AccountId},
...
-spec publish_queue_member_add(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kapps_call:call(),
    kz_term:api_non_neg_integer(),
    boolean(),
    kz_term:api_binary()
) -> 'ok'.
...
-spec publish_queue_member_remove(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
publish_queue_member_remove(AccountId, QueueId, CallId) ->
    Prop = [
        {<<"Account-ID">>, AccountId},
...
-spec update_strategy_with_agent(
    ...
    kz_term:ne_binaries(),
    'add' | 'remove',
    'ringing' | 'busy' | 'undefined'
) ->
    strategy_state().
update_strategy_with_agent(
    #state{
        strategy = S,
        strategy_state = SS
    },
...
-spec update_rr_strategy_with_agent(
    strategy_state(),
    kz_term:ne_binary(),
    agent_priority(),
    'add' | 'remove',
...
update_rr_strategy_with_agent(
    ...
    SS1 =
        case
            pqueue4:remove_unique(
                fun
                    (AgentId1) when AgentId =:= AgentId1 ->
                        'true';
                    (_) ->
                        'false'
                end,
        ...
        of
            ...
            {'false', AgentQueue1} ->
                ...
                SS#strategy_state{
                    agents = pqueue4:in(AgentId, Priority, AgentQueue1),
                    details = incr_agent(AgentId, Details)
                }
        end,
    set_flag(AgentId, Flag, SS1);
update_rr_strategy_with_agent(
    #strategy_state{agents = AgentQueue} = SS, AgentId, _Priority, 'remove', Flag
) ->
    SS1 =
        case lists:member(AgentId, pqueue4:to_list(AgentQueue)) of
            'false' ->
                SS;
            'true' ->
...
-spec update_mi_strategy_with_agent(
    strategy_state(), kz_term:ne_binary(), 'add' | 'remove', 'ringing' | 'busy' | 'undefined'
) ->
    strategy_state().
update_mi_strategy_with_agent(
    #strategy_state{
        agents = AgentL,
        details = Details
    } = SS,
...
update_sbrrss_with_agent(
    ...
    SS1 =
        ...
        of
            {'true', RRQueue1} ->
                lager:info("re-adding agent ~s (prio ~b) to strategy sbrr with skills ~p", [
                    AgentId, -1 * Priority, Skills
                ]),
                SS#strategy_state{
                    agents = SBRRSS#{
                        rr_queue := pqueue4:in(AgentId, Priority, RRQueue1),
                        skill_map := update_skill_map_with_agent(AgentId, Skills, SkillMap)
                    }
                };
            {'false', RRQueue1} ->
                lager:info("adding agent ~s (prio ~b) to strategy sbrr with skills ~p", [
                    AgentId, -1 * Priority, Skills
                ]),
                SS#strategy_state{
                    agents = SBRRSS#{
...
->
    case dict:find(AgentId, Details) of
        ...
        _ ->
            {_, AgentQueue1} = pqueue4:remove_unique(
                fun
                    (AgentId1) when AgentId1 =:= AgentId -> 'true';
                    (_) -> 'false'
                end,
                AgentQueue
            ),
...
remove_agent(
    ...
    case dict:find(AgentId, Details) of
        {'ok', {Count, _}} when Count > 1 ->
            SS#strategy_state{details = decr_agent(AgentId, Details)};
        _ ->
            {_, RRQueue1} = pqueue4:remove_unique(
                fun
                    (AgentId1) when AgentId1 =:= AgentId -> 'true';
                    (_) -> 'false'
                end,
            ...
            %% Reseed the map assigning calls to agents since agents changed.
            SBRRSS1 = reseed_sbrrss_maps(
                SS1#strategy_state.agents, ss_size('sbrr', SS1, 'free'), Calls
            ),
...
decr_agent(AgentId, Details) ->
    dict:update(
        AgentId,
        fun
            ({Count, Flag}) when Count > 1 -> {Count - 1, Flag};
            ({_, Flag}) -> {0, Flag}
        end,
        {0, 'undefined'},
        Details
...
-spec set_flag(kz_term:ne_binary(), 'ringing' | 'busy' | 'undefined', strategy_state()) ->
    strategy_state().
set_flag(
    AgentId,
    Flag,
    ...
    SS#strategy_state{
        details = dict:update(AgentId, fun({Count, _}) -> {Count, Flag} end, {0, Flag}, Details),
        ringing_agents = RingingAgents1,
        busy_agents = BusyAgents1
    }.

-spec update_skill_map_with_agent(kz_term:ne_binary(), kz_term:ne_binaries(), sbrr_skill_map()) ->
    sbrr_skill_map().
...
-spec skill_combinations(kz_term:ne_binaries(), [kz_term:ne_binaries(), ...]) ->
    [kz_term:ne_binaries(), ...].
skill_combinations([], Combos) ->
    Combos;
skill_combinations([Skill | Skills], Combos) ->
    CombosWithSkillPrefix = lists:map(
        fun(Combo) ->
            [Skill | Combo]
        end,
        Combos
...
maybe_reseed_sbrrss_maps(State) ->
    State.
...
-spec do_reseed_sbrrss_maps(sbrr_strategy_state(), sets:set(), non_neg_integer(), list()) ->
    sbrr_strategy_state().
...
do_reseed_sbrrss_maps(SBRRSS, _, _, []) ->
    ...
    SBRRSS;
do_reseed_sbrrss_maps(#{skill_map := SkillMap} = SBRRSS, AssignedAgentIds, MaxAssignments, [
    {_, Call} | OtherCalls
]) ->
    ...
    case maps:is_key(Skills, SkillMap) of
        'true' ->
            ...
            case sbrrss_maybe_assign_agent(SBRRSS, Candidates, Call, OtherCalls) of
                ...
                {SBRRSS1, AgentId} ->
                    do_reseed_sbrrss_maps(
                        SBRRSS1,
                        sets:add_element(AgentId, AssignedAgentIds),
                        MaxAssignments - 1,
                        OtherCalls
                    )
            end;
...
sbrrss_maybe_assign_agent(
    #{
        agent_id_map := AgentIdMap,
    ...
    case sbrrss_assign_agent(SBRRSS, Candidates, Call, OtherCalls) of
        ...
        AgentId ->
            ...
            {
                SBRRSS#{
                    agent_id_map := AgentIdMap#{AgentId => CallId},
...
%% @end
%%------------------------------------------------------------------------------
-spec sbrrss_assign_agent(sbrr_strategy_state(), sets:set(), kapps_call:call(), list()) ->
    api_kz_term:ne_binary().
sbrrss_assign_agent(#{skill_map := SkillMap} = SBRRSS, Candidates, Call, [
    {_, OtherCall} | OtherCalls
]) ->
    case sets:size(Candidates) of
        ...
        _ ->
            ...
            Candidates1 = sets:subtract(
                Candidates, maps:get(OtherCallSkills, SkillMap, sets:new())
            ),
...
sbrrss_assign_agent(#{rr_queue := RRQueue}, Candidates, _, []) ->
    case sets:size(Candidates) of
        ...
        _ ->
            ...
            RRQueueCandidates = pqueue4:filter(
                fun(AgentId) -> sets:is_element(AgentId, Candidates) end, RRQueue
            ),
...
sort_agent(A, B) ->
    sort_agent2(
        kz_json:get_integer_value(<<"Idle-Time">>, A),
...
create_strategy_state(S, #strategy_state{agents = 'undefined'} = SS, AccountDb, QueueId) when
    S =:= 'rr' orelse
        S =:= 'all'
->
    create_strategy_state(S, SS#strategy_state{agents = pqueue4:new()}, AccountDb, QueueId);
create_strategy_state(S, #strategy_state{agents = AgentQ} = SS, AccountDb, QueueId) when
    S =:= 'rr' orelse
        S =:= 'all'
->
    case acdc_util:agents_in_queue(AccountDb, QueueId) of
        [] ->
            lager:debug("no agents around"),
            SS;
        {'error', _E} ->
            lager:debug("error creating strategy rr: ~p", [_E]),
            SS;
        JObjs ->
            AgentMap = lists:map(
                fun(JObj) ->
            ...
            Q1 = lists:foldl(
                fun({AgentId, Priority}, Q) ->
                    lager:info("adding agent ~s (prio ~b) to strategy ~s", [
                        AgentId, -1 * Priority, S
                    ]),
...
create_strategy_state('mi', #strategy_state{agents = AgentL} = SS, AccountDb, QueueId) ->
    case acdc_util:agents_in_queue(AccountDb, QueueId) of
        [] ->
            ...
            SS;
        {'error', _E} ->
            lager:debug("error creating strategy mi: ~p", [_E]),
            SS;
        JObjs ->
            AgentL1 = lists:foldl(
                fun(JObj, Acc) ->
...
create_strategy_state('sbrr', #strategy_state{agents = 'undefined'} = SS, AccountDb, QueueId) ->
    ...
    create_strategy_state(
        'sbrr', SS#strategy_state{agents = SBRRStrategyState}, AccountDb, QueueId
    );
create_strategy_state('sbrr', SS, AccountDb, QueueId) ->
    case acdc_util:agents_in_queue(AccountDb, QueueId) of
        ...
        {'error', _E} ->
            ...
            SS;
        JObjs ->
            lists:foldl(fun update_sbrrss_with_agent/2, SS, JObjs)
    end.

update_strategy_state(Srv, S, #strategy_state{agents = AgentQueue}) when
    S =:= 'rr' orelse
        S =:= 'all'
->
...
-spec maybe_schedule_position_announcements(kz_json:object(), kapps_call:call(), mgr_state()) ->
    mgr_state().
maybe_schedule_position_announcements(
    JObj,
    Call,
    #state{
    ...
    case
        not EnterAsCallback andalso
            acdc_announcements_sup:maybe_start_announcements(self(), Call, AnnouncementsConfig)
    of
        'false' ->
            State;
        {'ok', Pid} ->
...
-spec cancel_position_announcements(kapps_call:call() | 'undefined', map()) ->
    map().
cancel_position_announcements('undefined', Pids) ->
    Pids;
...
-spec maybe_add_queue_member_as_callback(kz_json:object(), kapps_call:call(), mgr_state()) ->
    mgr_state().
maybe_add_queue_member_as_callback(
    JObj,
    Call,
    #state{
        account_id = AccountId,
...
callback_flag(AccountId, QueueId, Call) ->
    ...
    _ = acdc_stats:call_marked_callback(
        AccountId,
        QueueId,
