...
db_name(Account, {Yr, Mn}) ->
    kz_util:format_account_mod_id(Account, Yr, Mn);
...
call_summary_req(JObj) ->
    ...
    Queues =
        case kz_json:get_value(<<"Queue-ID">>, JObj) of
            undefined ->
                [
                    {A, Q, StartRange, EndRange}
                 || {_, {A, Q, _}} <- acdc_queues_sup:queues_running(), A == AccountId
                ];
            Else ->
                [{AccountId, Else, StartRange, EndRange}]
        end,
...
query_call_summary(Queues) ->
    QueryResults =
        lists:filter(
            fun(X) -> not kz_json:is_empty(X) end,
        ...
        fun(QR, JObj) ->
            ...
            QueueJObj = kz_json:set_values(
                [
                    ...
                    {<<"average_wait_time">>,
                        kz_json:get_value(<<"wait_time">>, QR) div TotalCalls},
                    {<<"average_talk_time">>,
                        kz_json:get_value(<<"talk_time">>, QR) div
...
merge_results(JObj1, JObj2) ->
    Fun = fun
        (_, {both, V1, V2}) when is_integer(V1), is_integer(V2) -> {ok, V1 + V2};
        (_, {both, V, V}) -> {ok, V};
        (_, {_, V}) -> {ok, V}
    end,
...
modb_db_list(AccountId, Next, End, Acc) when
    Next =:= End
->
...
-spec publish_call_query_errors(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:proplist() | {'error', _}
) -> 'ok'.
...
-spec publish_call_summary_query_errors(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:proplist() | {'error', _}
) -> 'ok'.
