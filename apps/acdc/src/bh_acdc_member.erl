...
init() ->
    init_bindings(),
    _ = blackhole_bindings:bind(
        <<"blackhole.events.validate.acdc_member_call">>, ?MODULE, 'validate'
    ),
    _ = blackhole_bindings:bind(
        <<"blackhole.events.bindings.acdc_member_call">>, ?MODULE, 'bindings'
    ),
    _ = blackhole_bindings:bind(
        <<"blackhole.events.validate.acdc_member_call_result">>, ?MODULE, 'validate'
    ),
    blackhole_bindings:bind(
        <<"blackhole.events.bindings.acdc_member_call_result">>, ?MODULE, 'bindings'
    ).
...
validate(Context, #{keys := Keys}) ->
    bh_context:add_error(
        Context, <<"invalid format for ACDC member subscription : ", (kz_binary:join(Keys))/binary>>
    ).
