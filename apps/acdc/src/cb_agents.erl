...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.content_types_provided.agents">>, ?MODULE, 'content_types_provided'
    ),
...
content_types_provided(Context, ?STATUS_PATH_TOKEN) ->
    Context;
...
post(Context, AgentId, ?STATUS_PATH_TOKEN) ->
    case cb_context:req_value(Context, <<"status">>) of
        <<"login">> ->
            publish_update(Context, AgentId, fun kapi_acdc_agent:publish_login/1);
        <<"logout">> ->
            publish_update(Context, AgentId, fun kapi_acdc_agent:publish_logout/1);
        <<"pause">> ->
            publish_update(Context, AgentId, fun kapi_acdc_agent:publish_pause/1);
        <<"resume">> ->
            publish_update(Context, AgentId, fun kapi_acdc_agent:publish_resume/1);
        <<"end_wrapup">> ->
            publish_update(Context, AgentId, fun kapi_acdc_agent:publish_end_wrapup/1)
    end,
...
-spec fetch_agent_stats(kz_term:api_binary(), cb_context:context(), boolean()) ->
    cb_context:context().
...
-spec fetch_stats_summary_from_amqp(cb_context:context(), kz_term:kz_proplist()) ->
    cb_context:context().
...
-spec fetch_current_agent_stats(kz_term:api_binary() | 'all', cb_context:context(), boolean()) ->
    cb_context:context().
...
-spec fetch_current_status(cb_context:context(), kz_term:api_binary(), kz_term:api_binary()) ->
    cb_context:context().
...
-spec fetch_ranged_agent_stats(
    kz_term:api_binary(), cb_context:context(), pos_integer(), boolean()
) -> cb_context:context().
...
-spec fetch_ranged_agent_stats(
    kz_term:api_binary(), cb_context:context(), pos_integer(), pos_integer(), boolean(), boolean
) ->
    cb_context:context().
...
-spec fetch_stats_from_amqp(cb_context:context(), kz_term:kz_proplist(), boolean()) ->
    cb_context:context().
...
format_stats(Context, Resp) ->
    Stats =
        kz_doc:public_fields(kz_json:get_value(<<"Handled">>, Resp, [])) ++
...
validate_status_change(Context, S) ->
    case lists:member(S, ?STATUS_CHANGES) of
        'true' ->
            validate_status_change_params(Context, S);
        'false' ->
