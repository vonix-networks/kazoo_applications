...
-define(STAT_TIMESTAMP_KEYS, [
    ?STAT_TIMESTAMP_PROCESSED,
...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.content_types_provided.queues">>, ?MODULE, 'content_types_provided'
    ),
...
content_types_provided(Context, ?STATS_SUMMARY_PATH_TOKEN) ->
    Context;
content_types_provided(Context, ?EAVESDROP_PATH_TOKEN) ->
    Context.
...
validate_eavesdrop_on_call(Context) ->
    ...
    Fs = [
        {fun is_valid_endpoint/2, [Context, Data]},
...
put(Context, ?EAVESDROP_PATH_TOKEN) ->
    Prop = [
        {<<"Eavesdrop-Call-ID">>, cb_context:req_value(Context, <<"call_id">>)}
...
default_eavesdrop_req(Context) ->
    [
        ...
        {<<"Endpoint-Timeout">>,
            kz_term:to_integer(cb_context:req_value(Context, <<"timeout">>, 20))},
...
eavesdrop_req(Context, Prop) ->
    ...
    of
        {'ok', Resp} ->
            crossbar_util:response(filter_response_fields(Resp), Context);
        {'error', 'timeout'} ->
        ...
        {'error', E} ->
            crossbar_util:response('error', <<"error">>, 500, E, Context)
    end.
...
add_queue_to_agents(Id, Context, AgentIds) ->
    ...
    {InQueueAgents, RmAgentIds} = lists:partition(
        fun(A) -> lists:member(A, AgentIds) end, CurrAgentIds
    ),
...
maybe_add_queue_to_agent(Id, A) ->
    Qs =
        case kz_json:get_value(<<"queues">>, A) of
            L when is_list(L) ->
...
-spec maybe_rm_agents(kz_term:ne_binary(), cb_context:context(), kz_json:path()) ->
    cb_context:context().
...
-spec fetch_stats_summary(cb_context:context(), kz_term:ne_binary() | 'all') ->
    cb_context:context().
...
-spec fetch_current_stats_summary(cb_context:context(), kz_term:ne_binary() | 'all') ->
    cb_context:context().
    ...
    case kz_term:to_integer(StartRange) of
        ...
        F when (To - F) >= MaxRange ->
            ...
            Msg = kz_term:to_binary(
                io_lib:format("end_range ~b is more than ~b seconds from start_range ~b", [
                    To, MaxRange, F
                ])
            ),
...
fetch_ranged_queue_stats(Context, QueueId, From, To, 'true') ->
    lager:debug("ranged query from ~b to ~b(~b) of current stats (now ~b)", [
        From, To, To - From, kz_time:current_tstamp()
    ]),
    Req = props:filter_undefined(
        [
            {<<"Account-ID">>, cb_context:account_id(Context)},
...
-spec fetch_current_queue_stats(cb_context:context(), kz_term:ne_binary() | 'all') ->
    cb_context:context().
...
format_stats(Context, Resp) ->
    ...
    RespData = kz_json:from_list([
        {<<"current_timestamp">>, kz_time:current_tstamp()},
        {<<"stats">>, Filtered}
    ]),
...
-spec fetch_call_summary_stats_from_amqp(cb_context:context(), kz_term:proplist()) ->
    cb_context:context().
fetch_call_summary_stats_from_amqp(Context, Req) ->
    case
        kz_amqp_worker:call(
    ...
    case kz_datamgr:open_cache_doc(?KZ_ACDC_DB, cb_context:account_id(Context)) of
        ...
        {'error', 'not_found'} ->
            ...
            Doc = kz_doc:update_pvt_parameters(
                kz_json:from_list([{<<"_id">>, cb_context:account_id(Context)}]),
                ?KZ_ACDC_DB,
                [
