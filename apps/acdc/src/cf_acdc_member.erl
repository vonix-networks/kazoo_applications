...
handle(Data, Call) ->
    ...
    MaxQueueSize = max_queue_size(
        kz_json:get_integer_value(<<"max_queue_size">>, QueueJObj, 'undefined')
    ),

    Call1 = maybe_enable_callback(
        kapps_call:kvs_store_proplist(
    ...
    lager:info("member call: ~s enter queue: ~s  max_wait: ~p (secs) max size: ~p curr size: ~p", [
        kapps_call:call_id(Call),
        QueueId,
        MaxWait div ?MILLISECONDS_IN_SECOND,
        MaxQueueSize,
        CurrQueueSize
    ]),
    ...
    maybe_enter_queue(
        #member_call{
            ...
            breakout_media = kz_json:get_value(
                [<<"breakout">>, <<"media">>], QueueJObj, kz_json:new()
            ),
            queue_id = QueueId,
            max_wait = MaxWait,
...
maybe_enable_callback(Call, QueueJObj) ->
    RestrictedClassifiers = kz_json:get_json_value(
        [<<"breakout">>, <<"classifiers">>], QueueJObj, kz_json:new()
    ),
    ...
    case
        BreakoutKey =/= 'undefined' andalso
            not callback_restricted(RestrictedClassifiers, CallerClassification)
    of
        'true' ->
...
maybe_enter_queue(
    ...
    case callback_restricted(RestrictedClassifiers, CallerClassification) of
        'false' ->
            ...
            kapps_call_command:prompt(
                breakout_prompt(BreakoutMedia), kapps_call:language(Call), Call
            ),
            enter_as_callback_loop(MC, #breakout_state{});
        'true' ->
            lager:info("queue restricted callback from caller with classification \"~s\"", [
...
maybe_enter_queue(
    ...
    case kapps_call_command:b_channel_status(kapps_call:call_id(Call)) of
        {'ok', _} ->
            lager:info("asking for an agent, waiting up to ~p secs", [
                MaxWait div ?MILLISECONDS_IN_SECOND
            ]),
            cf_exe:amqp_send(Call, MemberCall, fun kapi_acdc_queue:publish_member_call/1),
            _ = kapps_call_command:flush_dtmf(Call),
            wait_for_bridge(
                MC#member_call{call = kapps_call:kvs_store('queue_id', QueueId, Call)},
                #breakout_state{},
                MaxWait
            );
        {'error', E} ->
...
wait_for_bridge(#member_call{call = Call} = MC, BreakoutState, Timeout, Start) ->
    ...
    receive
        {'amqp_msg', JObj} ->
            process_message(
                MC, BreakoutState, Timeout, Start, Wait, JObj, kz_util:get_event_type(JObj)
            )
    after Timeout ->
...
process_message(
    #member_call{call = Call}, _, _, Start, _Wait, _JObj, {<<"call_event">>, <<"CHANNEL_BRIDGE">>}
) ->
    lager:info("member was bridged to agent, yay! took ~b s", [kz_time:elapsed_s(Start)]),
    cf_exe:control_usurped(Call);
process_message(
    #member_call{call = Call}, _, _, Start, _Wait, _JObj, {<<"call_event">>, <<"CHANNEL_DESTROY">>}
) ->
    lager:info("member hungup while waiting in the queue (was there ~b s)", [
        kz_time:elapsed_s(Start)
    ]),
...
process_message(
    ...
    case QueueId =:= kz_json:get_value(<<"Queue-ID">>, JObj) of
        'true' ->
            ...
            lager:info(
                "call failed to be processed: ~s (took ~b s)",
...
process_message(
    #member_call{call = Call}, _, _, Start, _Wait, _JObj, {<<"member">>, <<"call_success">>}
) ->
...
-spec process_dtmf(
    binary(), member_call(), breakout_state(), max_wait(), kz_term:kz_now(), kz_term:kz_now()
) -> 'ok'.
process_dtmf(
    DTMF,
    #member_call{
    ...
    #breakout_state{active = 'false'} = BreakoutState,
    Timeout,
    Start,
    Wait
) ->
    ...
    case DTMF of
        ...
        BreakoutKey ->
            ...
            wait_for_bridge(
                MC,
                BreakoutState#breakout_state{active = 'true'},
...
-spec breakout_loop(binary(), member_call(), breakout_state()) ->
    breakout_state() | 'callback_registered' | 'cancel'.
...
-spec process_breakout_message(binary(), member_call(), breakout_state()) ->
    breakout_state() | 'callback_registered' | 'cancel'.
process_breakout_message(
    DTMF,
    #member_call{
        call = Call,
        breakout_media = BreakoutMedia
    ...
    case DTMF of
        <<"1">> ->
            ...
            breakout_number_correct(Call, BreakoutMedia, State#breakout_state{
                callback_number = From
            });
        <<"2">> ->
            'cancel';
        DTMF ->
            breakout_invalid_selection(Call, State, DTMF)
    end;
process_breakout_message(
    DTMF,
    #member_call{
    ...
    case DTMF of
        <<"1">> ->
            ...
            kapps_call_command:prompt(
                callback_registered(BreakoutMedia), kapps_call:language(Call), Call
            ),
...
process_breakout_message(
    ...
    case DTMF of
        ...
        _ ->
            State#breakout_state{callback_number = <<Number/binary, DTMF/binary>>}
    end.
...
-spec breakout_number_correct(kapps_call:call(), kz_term:ne_binary(), breakout_state()) ->
    breakout_state().
breakout_number_correct(Call, BreakoutMedia, #breakout_state{callback_number = Number} = State) ->
    Prompt = [
        {'prompt', call_back_at(BreakoutMedia)},
        {'say', Number},
...
current_queue_size(AccountId, QueueId) ->
    ...
    case hackney:request('get', URL, Headers, [], []) of
        ...
        _Else ->
            lager:warning(
                "rabbitMQ Management plugin problem, check that 'rabbitmq_management' is enabled in /etc/kazoo/rabbitmq/enabled_plugins"
            ),
            'undefined'
    end.
