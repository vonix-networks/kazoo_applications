...
-export([
    ...
    shared_originate_failure/1,
    shared_originate_failure_v/1,
    shared_call_id/1,
    shared_call_id_v/1
]).
...
-define(STATS_REQ_VALUES, [
    {<<"Event-Category">>, <<"agent">>},
    {<<"Event-Name">>, <<"stats_req">>}
]).
-define(STATS_REQ_TYPES, []).

-spec stats_req(kz_term:api_terms()) ->
    {'ok', iolist()}
    | {'error', string()}.
...
-spec stats_req_routing_key(kz_json:object() | kz_term:proplist() | kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
-spec stats_req_publish_key(kz_json:object() | kz_term:proplist() | kz_term:ne_binary()) ->
    kz_term:ne_binary().
stats_req_publish_key(Props) when is_list(Props) ->
    stats_req_routing_key(
...
-define(OPTIONAL_STATS_RESP_HEADERS, [
    <<"Current-Calls">>, <<"Current-Stats">>, <<"Current-Statuses">>, <<"Agent-Call-IDs">>
]).
-define(STATS_RESP_VALUES, [
    {<<"Event-Category">>, <<"agent">>},
...
% append queue ID
-define(AGENT_KEY, "acdc.agent.action.").

-define(AGENT_HEADERS, [<<"Account-ID">>, <<"Agent-ID">>]).
-define(OPTIONAL_AGENT_HEADERS, [
    <<"Queue-ID">>,
    <<"Presence-ID">>,
    <<"Presence-State">>
]).
...
end_wrapup(JObj) ->
    end_wrapup(kz_json:to_proplist(JObj)).
...
end_wrapup_v(JObj) ->
    end_wrapup_v(kz_json:to_proplist(JObj)).
...
restart(JObj) ->
    restart(kz_json:to_proplist(JObj)).
...
restart_v(JObj) ->
    restart_v(kz_json:to_proplist(JObj)).
...
-spec agent_status_routing_key(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
shared_originate_failure(Props) when is_list(Props) ->
    case shared_originate_failure_v(Props) of
        'true' ->
            kz_api:build_message(Props, ?SHARED_FAILURE_HEADERS, ?OPTIONAL_SHARED_FAILURE_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for shared_originate_failure"}
    end;
shared_originate_failure(JObj) ->
    shared_originate_failure(kz_json:to_proplist(JObj)).
...
shared_originate_failure_v(JObj) ->
    shared_originate_failure_v(kz_json:to_proplist(JObj)).
...
shared_call_id(Props) when is_list(Props) ->
    case shared_call_id_v(Props) of
        'true' ->
            kz_api:build_message(Props, ?SHARED_CALL_ID_HEADERS, ?OPTIONAL_SHARED_CALL_ID_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for shared_call_id"}
    end;
shared_call_id(JObj) ->
    shared_call_id(kz_json:to_proplist(JObj)).
...
shared_call_id_v(JObj) ->
    shared_call_id_v(kz_json:to_proplist(JObj)).
...
unbind_q(Q, {<<"*">>, <<"*">>, CallId, _} = Ids, ['stats_req' | T]) ->
    _ = kz_amqp_util:unbind_q_from_kapps(
        Q, stats_req_routing_key('undefined', 'undefined', CallId)
    ),
    unbind_q(Q, Ids, T);
...
publish_sync_resp('undefined', _, _) ->
...
publish_shared_originate_failure(API, ContentType) ->
    {'ok', Payload} = shared_originate_failure(
        (API1 = kz_api:prepare_api_payload(API, ?SHARED_FAILURE_VALUES))
    ),
...
publish_shared_call_id(API, ContentType) ->
    {'ok', Payload} = shared_call_id(
        (API1 = kz_api:prepare_api_payload(API, ?SHARED_CALL_ID_VALUES))
    ),
    kz_amqp_util:kapps_publish(fsm_shared_routing_key(API1), Payload, ContentType).
