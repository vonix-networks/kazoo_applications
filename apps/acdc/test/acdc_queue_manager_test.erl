...
apply_n({Fun, Args}, State, N) -> apply_n({Fun, Args}, apply(Fun, [State | Args]), N - 1).
...
add_agent({AgentId, Skills}, State) ->
    SS = acdc_queue_manager:update_strategy_with_agent(
        State, AgentId, 0, Skills, 'add', 'undefined'
    ),
...
add_call({CallId, Skills}, #state{current_member_calls = Calls} = State) ->
    ...
    Call = kapps_call:set_call_id(
        CallId, kapps_call:kvs_store(?ACDC_REQUIRED_SKILLS_KEY, Skills1, kapps_call:new())
    ),
    Calls1 = Calls ++ [{0, Call}],
    State1 =
        #state{strategy_state = #strategy_state{agents = SBRRSS} = SS} = State#state{
            current_member_calls = Calls1
        },
    SBRRSS1 = acdc_queue_manager:reseed_sbrrss_maps(
        SBRRSS, acdc_queue_manager:ss_size('sbrr', SS, 'free'), Calls1
    ),
