...
help_10392_test_() ->
    {'setup', fun() -> kz_fixturedb_util:start_me('true') end, fun kz_fixturedb_util:stop_me/1, fun(
            ...
            [
                ...
                {"HELP-10392: Sort endpoints by delay (Ring strategy=simultaneous (at the same time))",
                    get_endpoints()},
...
help_10392() ->
    ...
    EndpointDelay10 =
        {?HELP_10392_DEVICE_ID, help_10392_add_source(help_10392_add_group_weight(Endpoint))},
...
get_endpoints() ->
    ...
    Resp6 = cf_ring_group:get_endpoints(
        help_10392_data([MemberGroup10, MemberUser0, MemberGroup10]), Call
    ),
    Resp7 = cf_ring_group:get_endpoints(
        help_10392_data([MemberUser10, MemberGroup0, MemberUser10]), Call
    ),
    Resp8 = cf_ring_group:get_endpoints(
        help_10392_data([MemberGroup0, MemberUser10, MemberGroup0]), Call
    ),
...
get_endpoints_in_order() ->
    ...
    Resp0 = cf_ring_group:get_endpoints(
        help_10392_data([MemberUser10, MemberUser0], <<"single">>), Call
    ),
    Resp1 = cf_ring_group:get_endpoints(
        help_10392_data([MemberUser0, MemberUser10], <<"single">>), Call
    ),
    Resp2 = cf_ring_group:get_endpoints(
        help_10392_data([MemberGroup0, MemberGroup10], <<"single">>), Call
    ),
    Resp3 = cf_ring_group:get_endpoints(
        help_10392_data([MemberGroup10, MemberGroup0], <<"single">>), Call
    ),
...
match_jobjs_delay([Expected | Tail0], [Received | Tail1], _) ->
    case
        kz_json:are_json_objects([Expected, Received]) andalso
            same_delay(Expected, Received)
    of
        'false' -> 'false';
...
help_10392_call() ->
    Routines = [
        {fun kapps_call:set_account_id/2, ?HELP_10392_ACCOUNT_ID},
