...
check_no_plans_financials() ->
    ...
    [
        {'setup', fun() -> [meck:new(M, ['passthrough']) || M <- ToMeck] end,
            fun(_) -> [meck:unload(Mecked) || Mecked <- ToMeck] end, fun(_) ->
                Context = cb_context:set_resp_data(cb_context:new(), []),

                %% simulate account has no plans
                meck:expect('kapps_util', 'get_master_account_id', fun() ->
...
check_low_balance() ->
    ...
    [
        ...
        {"If threshold not configured, postpay disabled, and current balance == 0 raise an alert",
            ?_assertEqual(
                {ZeroBalance, ThresholdNotConfigured, <<"low_balance">>},
                {
        ...
        {"If threshold not configured, postpay disabled, and current balance < 0 raise an alert",
            ?_assertEqual(
                {NegativeBalance, ThresholdNotConfigured, <<"low_balance">>},
                {
        ...
        {"If threshold not configured, postpay enabled, and current balance < PostPayAmountUSD raise an alert",
            ?_assertEqual(
                {BelowPostPayAmountUSD, PostPayAmountUSD, <<"low_balance">>},
                {
        ...
        {"If threshold not configured, postpay enabled, and current balance == PostPayAmountUSD raise an alert",
            ?_assertEqual(
                {PostPayAmountUSD, PostPayAmountUSD, <<"low_balance">>},
                {
        ...
        {"If threshold configured and current balance < threshold raise an alert",
            ?_assertEqual(
                {BelowThresholdUSD, ThresholdUSD, <<"low_balance">>},
                {
        ...
        {"Validate mecked modules", ?_assert(MeckValidate)}
    ].
    ...
    [
        ...
        },
        {
            "If account has service plans assigned + default payment tokens configured and " ++
                "any of those tokens has expired or is about to expire raise an alert",
            ?_assertEqual(
                {<<"expired_payment_token">>, <<"expired_payment_token">>},
...
expired_payment_token() ->
    ...
    %% Expired 1 day ago.
    Expiration = kz_time:now_s() - ?SECONDS_IN_DAY,
...
about_to_expire_payment_token() ->
    ...
    Expiration = kz_time:now_s() + (?SECONDS_IN_DAY * 30),
    {TokenId, kz_json:set_value(<<"expiration">>, Expiration, Token)}.
