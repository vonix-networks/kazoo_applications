...
allow_accounts_test_() ->
    ...
    Accounts = [
        {?ACCOUNT_AUTH, ?DENY_REQ},
        %% auth can change descendant
        {?ACCOUNT_DESCENDANT, ?ALLOW_REQ},
        %% explicit account
        {?ACCOUNT_ID, ?ALLOW_REQ},
        %% explicit auth account
        {?AUTH_ACCOUNT_ID, ?DENY_REQ},
        %% missing "allowed_accounts"
        {'undefined', ?ALLOW_REQ}
    ],
    ...
    Context =
        cb_context:setters(
            ...
            [
                ...
                {fun cb_context:set_account_id/2, ?ACCOUNT_ID},
                {fun cb_context:set_req_verb/2, ?HTTP_GET},
                {fun cb_context:set_auth_doc/2, auth_doc(?ALLOW_ACCOUNTS_RESTRICTIONS(Account))}
            ]
        ),
    ...
    Context =
        cb_context:setters(
            ...
            [
                ...
                {fun cb_context:set_account_id/2, ?ACCOUNT_ID},
                {fun cb_context:set_req_verb/2, ?HTTP_GET},
                {fun cb_context:set_auth_doc/2, auth_doc(?ARGUMENTS_RESTRICTIONS(ArgPattern))}
            ]
        ),
    ...
    Context =
        cb_context:setters(
            ...
            [
                ...
                {fun cb_context:set_account_id/2, ?ACCOUNT_ID},
                {fun cb_context:set_req_verb/2, ?HTTP_GET},
                {fun cb_context:set_auth_doc/2, auth_doc(?HTTP_VERB_RESTRICTIONS(Verbs))}
            ]
        ),
...
build_assertions(Label, Context, ExpectedResults) ->
    [
        {Label, ?_assertEqual(Result, maybe_deny_access(Context, ReqParams))}
