...
to_gen(Doc, {Before, Now, After}, Filter) ->
    [
        {
            test_desc(
                "Verify doc is removed when querying ~s_to with timestamp before doc's",
                [Filter]
            ),
            ?_assertEqual(
                'false', crossbar_filter:filter_doc_by_querystring(Doc, to_qs(Before, Filter))
            )
        },
        {
            test_desc(
                "Verify doc is kept when querying ~s_to with timestamp equal to doc's",
                [Filter]
            ),
            ?_assertEqual(
                'true', crossbar_filter:filter_doc_by_querystring(Doc, to_qs(Now, Filter))
            )
        },
        {
            test_desc(
                "Verify doc is kept when querying ~s_to with timestamp after doc's",
                [Filter]
            ),
            ?_assertEqual(
                'true', crossbar_filter:filter_doc_by_querystring(Doc, to_qs(After, Filter))
            )
        }
    ].
...
from_gen(Doc, {Before, Now, After}, Filter) ->
    [
        {
            test_desc(
                "Verify doc is kept when querying ~s_from with timestamp before doc's",
                [Filter]
            ),
            ?_assertEqual(
                'true', crossbar_filter:filter_doc_by_querystring(Doc, from_qs(Before, Filter))
            )
        },
        {
            test_desc(
                "Verify doc is kept when querying ~s_from with timestamp equal to doc's",
                [Filter]
            ),
            ?_assertEqual(
                'true', crossbar_filter:filter_doc_by_querystring(Doc, from_qs(Now, Filter))
            )
        },
        {
            test_desc(
                "Verify doc is removed when querying ~s_from with timestamp after doc's",
                [Filter]
            ),
            ?_assertEqual(
                'false', crossbar_filter:filter_doc_by_querystring(Doc, from_qs(After, Filter))
            )
        }
    ].
...
multi_filter(Doc) ->
    ...
    QSTrue = kz_json:decode(
        <<"{\"filter_foo\":\"bar\",\"created_from\":", CFrom/binary, ",\"created_to\":", CTo/binary,
            "}">>
    ),

    QSFalse = kz_json:decode(
        <<"{\"filter_foo\":\"bar\",\"created_from\":", CTo/binary, ",\"created_to\":", CFrom/binary,
            "}">>
    ),
