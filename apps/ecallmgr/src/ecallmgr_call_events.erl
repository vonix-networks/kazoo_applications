...
-define(DEBUG_CHANNEL,
    kapps_config:get_boolean(?APP_NAME, <<"debug_channel">>, ?DEFAULT_DEBUG_CHANNEL)
).
...
graceful_shutdown(Node, UUID) ->
    _ = [
        gen_server:cast(Pid, {'graceful_shutdown', UUID})
...
listen_for_other_leg(_Node, _UUID, 'undefined') ->
    'ok';
listen_for_other_leg(_Node, _UUID, []) ->
    'ok';
listen_for_other_leg(Node, UUID, [_ | _] = Events) ->
    _ = [
        gen_server:cast(Pid, {'b_leg_events', Events})
     || Pid <- gproc:lookup_pids({'p', 'l', ?FS_CALL_EVENT_REG_MSG(Node, UUID)})
    ],
...
handle_cast({'transferer', _Props}, State) ->
    ...
    {'stop', 'normal', State};
handle_cast(
    {'b_leg_events', NewEvents},
    #state{
        other_leg_events = Evts,
        other_leg = 'undefined'
    } = State
) ->
...
handle_cast(
    ...
    _Started =
        (Events =/= []) andalso
            ecallmgr_call_sup:start_event_process(Node, OtherLeg),
...
handle_info(
    ...
    case
        {
            props:get_first_defined([<<"Event-Subclass">>, <<"Event-Name">>], Props),
...
handle_info(
    {'event', [_WrongCallId | _Props]},
    #state{
        call_id = _CallId,
        other_leg = _OtherLeg
    } = State
) ->
    lager:debug("recv event for unknown ~s: ~s", [
        _WrongCallId, props:get_value(<<"Event-Name">>, _Props)
    ]),
...
) when (FNC + 1) > ?MAX_FAILED_NODE_CHECKS ->
    ...
    case ecallmgr_util:is_node_up(Node, CallId) of
        'true' ->
            ...
            {'noreply',
                ...
                'hibernate'};
        'false' ->
            lager:debug("node ~s still not up, waiting ~pms to test again", [
                Node, ?NODE_CHECK_PERIOD
            ]),
            TRef = erlang:send_after(?NODE_CHECK_PERIOD, self(), {'check_node_status'}),
            {'noreply',
                State#state{
...
handle_info('timeout', #state{failed_node_checks = FNC} = State) when
    (FNC + 1) > ?MAX_FAILED_NODE_CHECKS
->
...
handle_info(
    ...
    case freeswitch:api(Node, 'uuid_exists', CallId) of
        ...
        {'ok', <<"true">>} ->
            ...
            'true' = gproc:reg(
                {'p', 'l', ?FS_EVENT_REG_MSG(Node, ?CHANNEL_MOVE_RELEASED_EVENT_BIN)}
            ),
            _ = usurp_other_publishers(State),
...
handle_info('shutdown', State) ->
    {'stop', 'normal', State};
handle_info(
    ?LOOPBACK_BOWOUT_MSG(Node, Props),
    #state{
        call_id = ResigningUUID,
        node = Node
    } = State
) ->
...
handle_info(
    {'usurp_publisher', CallId, RefId, _JObj},
    #state{
        ref = RefId,
        call_id = CallId
    } = State
...
handle_bowout(Node, Props, ResigningUUID) ->
    case {props:get_value(?RESIGNING_UUID, Props), props:get_value(?ACQUIRED_UUID, Props)} of
        ...
        {ResigningUUID, AcquiringUUID} when AcquiringUUID =/= 'undefined' ->
            lager:debug(
                "loopback bowout detected, replacing ~s with ~s",
...
maybe_process_channel_destroy(Node, CallId, Props) ->
    ...
    case ecallmgr_fs_channel:node(CallId) of
        {'ok', Node} ->
            gen_server:cast(self(), {'graceful_shutdown', CallId});
        {'error', _} ->
            gen_server:cast(self(), {'graceful_shutdown', CallId});
        {'ok', _NewNode} ->
            lager:debug(
                "channel is on ~s, not ~s: publishing channel move",
...
-spec maybe_manual_bowout(atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
maybe_manual_bowout(Node, <<"att_xfer">>, <<"originator">>, UUID) ->
    case ecallmgr_fs_channel:fetch(UUID, 'record') of
        {'ok', #channel{loopback_other_leg = OtherLeg, is_loopback = 'true'}} ->
            ...
            _ = freeswitch:api(
                Node, 'uuid_setvar', <<OtherLeg/binary, " ", "loopback_bowout true">>
            ),
...
-spec create_event(kz_term:ne_binary(), kz_term:api_binary(), kz_term:proplist()) ->
    kz_term:proplist().
...
specific_call_channel_vars_props(_EventName, Props) ->
    [
        ...
        {<<"Custom-Application-Vars">>,
            kz_json:from_list(ecallmgr_util:custom_application_vars(Props))}
    ].
...
generic_call_event_props(Props) ->
    ...
    NormalizedFSTimestamp = kz_time:unix_seconds_to_gregorian_seconds(
        FSTimestamp div ?MICROSECONDS_IN_SECOND
    ),
    ...
    [
        ...
        {<<"Channel-Created-Time">>,
            props:get_integer_value(<<"Caller-Channel-Created-Time">>, Props)},
        {<<"Channel-Is-Loopback">>,
            get_is_loopback(props:get_value(<<"variable_is_loopback">>, Props))},
        {<<"Channel-Loopback-Bowout">>, props:get_is_true(<<"variable_loopback_bowout">>, Props)},
        {<<"Channel-Loopback-Bowout-Execute">>,
            props:get_is_true(<<"variable_loopback_bowout_on_execute">>, Props)},
        ...
        {<<"Other-Leg-Destination-Number">>,
            props:get_value(<<"Other-Leg-Destination-Number">>, Props)},
        ...
        {<<"Replaced-By">>,
            props:get_first_defined([<<"att_xfer_replaced_by">>, ?ACQUIRED_UUID], Props)},
...
publish_event(Props) ->
    ...
    case {ApplicationName, EventName} of
        {_, <<"dtmf">>} ->
            lager:debug(
                "publishing received DTMF digit ~s",
        ...
        {ApplicationName, <<"channel_execute_complete">>} ->
            ApplicationResponse = kz_term:to_lower_binary(
                props:get_value(<<"Application-Response">>, Props, <<>>)
            ),
            ApplicationData = props:get_value(<<"Raw-Application-Data">>, Props, <<>>),
            lager:debug("publishing call event ~s '~s(~s)' result: ~s", [
                EventName, ApplicationName, ApplicationData, ApplicationResponse
            ]);
        {ApplicationName, _} ->
            ApplicationData = props:get_value(<<"Raw-Application-Data">>, Props, <<>>),
            lager:debug("publishing call event ~s '~s(~s)'", [
                EventName, ApplicationName, ApplicationData
            ])
    end,
...
-spec specific_call_event_props(binary(), kz_term:api_binary(), kz_term:proplist()) ->
    kz_term:proplist().
...
specific_call_event_props(<<"CHANNEL_EXECUTE_COMPLETE">>, <<"bridge">>, Props) ->
    [
        ...
        {<<"Application-Response">>,
            props:get_value(<<"variable_originate_disposition">>, Props, <<"FAIL">>)}
    ];
...
specific_call_event_props(_, <<"play_and_get_digits">>, Props) ->
    [
        ...
        {<<"Application-Response">>,
            props:get_value(<<"variable_collected_digits">>, Props, <<"">>)}
    ];
...
-spec transfer_to(kz_term:proplist() | kz_term:api_binary()) ->
    {kz_term:api_binary(), kz_term:api_binary()}.
transfer_to(Props) when
    is_list(Props)
->
    transfer_to(props:get_value(<<"variable_transfer_to">>, Props));
transfer_to(<<"att:", TransferTo/binary>>) ->
    {<<"attended">>, TransferTo};
transfer_to(<<"blind:", TransferTo/binary>>) ->
    {<<"blind">>, TransferTo};
transfer_to(_) ->
    {'undefined', 'undefined'}.
...
fax_specific(Props) ->
    props:filter_undefined(
        [
            ...
            {<<"Fax-Transferred-Pages">>,
                props:get_value(<<"variable_fax_document_transferred_pages">>, Props)},
            {<<"Fax-Total-Pages">>,
                props:get_value(<<"variable_fax_document_total_pages">>, Props)},
            ...
            {<<"Fax-Local-Station-ID">>,
                props:get_value(<<"variable_fax_local_station_id">>, Props)},
            {<<"Fax-Remote-Station-ID">>,
                props:get_value(<<"variable_fax_remote_station_id">>, Props)},
            ...
            {<<"Fax-Image-Resolution">>,
                props:get_value(<<"variable_fax_image_resolution">>, Props)},
            {<<"Fax-File-Image-Resolution">>,
                props:get_value(<<"variable_fax_file_image_resolution">>, Props)},
            {<<"Fax-Image-Size">>, props:get_value(<<"variable_fax_image_size">>, Props)},
            {<<"Fax-Image-Pixel-Size">>,
                props:get_value(<<"variable_fax_image_pixel_size">>, Props)},
            {<<"Fax-File-Image-Pixel-Size">>,
                props:get_value(<<"variable_fax_file_image_pixel_size">>, Props)},
            {<<"Fax-Longest-Bad-Row-Run">>,
                props:get_value(<<"variable_fax_longest_bad_row_run">>, Props)},
...
should_publish(<<"CHANNEL_EXECUTE_COMPLETE">>, <<"intercept">>, 'false') ->
    lager:debug(
        "suppressing intercept execute complete in favour the kazoo masquerade of this event"
    ),
    'false';
should_publish(<<"CHANNEL_EXECUTE_COMPLETE">>, <<"execute_extension">>, 'false') ->
    lager:debug(
        "suppressing execute_extension execute complete in favour the kazoo masquerade of this event"
    ),
    'false';
...
silence_terminated(Prop) when is_list(Prop) ->
    case props:get_value(<<"variable_silence_hits_exhausted">>, Prop) of
        'undefined' ->
            silence_terminated(props:get_integer_value(<<"variable_record_silence_hits">>, Prop));
        Ex ->
            kz_term:is_true(Ex)
    end.
    ...
    case
        [
         ...
         || Trnsf <- SerializedHistory,
            (HistJObj = create_trnsf_history_object(binary:split(Trnsf, <<":">>, ['global']))) =/=
                'undefined'
        ]
    of
        [] -> 'undefined';
...
get_duration_seconds(Props) ->
    ensure_non_negative(
        <<"variable_duration">>, props:get_integer_value(<<"variable_duration">>, Props, 0)
    ).
...
get_billing_seconds(Props) ->
    {Var, BillingSeconds} =
        case props:get_integer_value(<<"variable_billmsec">>, Props) of
            'undefined' ->
                {<<"variable_billsec">>, props:get_integer_value(<<"variable_billsec">>, Props, 0)};
            Billmsec ->
                {<<"variable_billmsec">>, kz_term:ceiling(Billmsec / 1000)}
        end,
...
get_progress_seconds(Props) ->
    ensure_non_negative(
        <<"variable_progresssec">>, props:get_integer_value(<<"variable_progresssec">>, Props, 0)
    ).
...
ensure_non_negative(Var, NegativeS) when NegativeS < 0 ->
    ...
    0;
ensure_non_negative(_Var, NonNegativeS) ->
    NonNegativeS.
...
debug_channel_props(Props, 'true') ->
    [{<<"Channel-Debug">>, kz_json:from_list(lists:sort(fun sort_debug/2, Props))}].
