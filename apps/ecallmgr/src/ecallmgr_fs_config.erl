...
handle_info(
    {'fetch', 'configuration', <<"configuration">>, <<"name">>, Conf, ID, []},
    ...
    {'noreply', State};
handle_info(
    {'fetch', 'configuration', <<"configuration">>, <<"name">>, Conf, ID, ['undefined' | Data]},
    #state{node = Node} = State
) ->
    lager:debug("fetch configuration request from ~s: ~s", [Node, ID]),
    ...
    {'noreply', State};
handle_info(
    {'fetch', 'configuration', <<"configuration">>, <<"name">>, Conf, ID, [UUID | Data]},
    #state{node = Node} = State
) when
    is_binary(UUID)
->
    ...
    {'noreply', State};
handle_info(
    {'fetch', 'configuration', <<"configuration">>, <<"name">>, Conf, ID, Data},
    #state{node = Node} = State
) when
    is_list(Data)
->
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_config_req(
    atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist() | 'undefined'
) -> fs_sendmsg_ret().
...
default_sip_profiles(Node) ->
    Gateways =
        case kapps_config:is_true(?APP_NAME, <<"process_gateways">>) of
            'false' ->
                kz_json:new();
            'true' ->
    ...
    JObj = kz_json:from_list([
        {kz_term:to_binary(?DEFAULT_FS_PROFILE), kz_json:from_list(default_sip_profile())}
    ]),
    kz_json:set_value(
        [kz_term:to_binary(?DEFAULT_FS_PROFILE), <<"Gateways">>],
        Gateways,
        JObj
    ).
...
maybe_kill_removed_gateways([], _, _) ->
    'ok';
maybe_kill_removed_gateways([GatewayName | Names], JObj, Node) ->
    _ =
        case kz_json:get_value(GatewayName, JObj) of
            'undefined' -> kill_gateway(GatewayName, Node);
            _Else -> 'ok'
        end,
    maybe_kill_removed_gateways(Names, JObj, Node).

-spec maybe_kill_changed_gateways(
    kz_term:ne_binaries(), kz_json:object(), kz_json:object(), atom()
) -> 'ok'.
maybe_kill_changed_gateways([], _, _, _) ->
    'ok';
...
-spec maybe_kill_changed_gateway(kz_term:ne_binary(), kz_json:object(), kz_json:object(), atom()) ->
    'ok'.
maybe_kill_changed_gateway(_, 'undefined', _, _) ->
    'ok';
...
fix_conference_profile(Name, Profile, FSNode) ->
    ...
    Routines = [
        fun(J) -> maybe_fix_profile_tts(J, FSNode) end,
...
-spec maybe_convert_sound(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) -> kz_json:object().
...
-spec fetch_conference_config(atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    fs_sendmsg_ret().
...
-spec fetch_conference_params(
    atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) -> fs_sendmsg_ret().
...
-spec maybe_fetch_conference_profile(
    atom(), kz_term:ne_binary(), kzd_freeswitch:doc(), kz_term:api_binary()
) -> fs_sendmsg_ret().
...
-spec fetch_mod_kazoo_config(atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    fs_sendmsg_ret().
...
-spec fetch_mod_kazoo_config_action(
    atom(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:proplist()
) -> fs_sendmsg_ret().
