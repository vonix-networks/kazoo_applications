...
-define(RESPONDERS, [
    {{?MODULE, 'handle_originate_execute'}, [{<<"dialplan">>, <<"originate_execute">>}]},
...
handle_call_events(JObj, Props) ->
    ...
    case
        props:get_value('uuid', Props) =:= kz_json:get_binary_value(<<"Call-ID">>, JObj) andalso
...
handle_cast(
    ...
    case
        kz_json:is_true(<<"Start-Control-Process">>, JObj, 'true') andalso
            start_control_process(State#state{uuid = UUID})
    of
        'false' ->
...
handle_cast(
    ...
    {'noreply', State#state{
        dialstrings = build_originate_args_from_endpoints(?ORIGINATE_PARK, Endpoints, JObj, FetchId)
    }};
handle_cast(
    {'build_originate_args'},
    #state{
...
handle_cast(
    ...
    case originate_execute(Node, Dialstrings, find_originate_timeout(JObj)) of
        ...
        {'ok', WinningUUID} when is_pid(CtrlPid) ->
            ...
            _ =
                case lists:member(CtrlPid, CtrlPids) of
                    'true' -> 'ok';
...
get_originate_action(<<"eavesdrop">>, JObj) ->
    ...
    case ecallmgr_fs_channel:node(EavesdropCallId) of
        {'error', _} ->
            lager:debug("failed to find channel ~p in node list", [
                kz_json:get_value(<<"Eavesdrop-Call-ID">>, JObj)
            ]),
            <<"error">>;
        {'ok', N} ->
...
get_transfer_action(_JObj, 'undefined') ->
    <<"error">>;
...
-spec intercept_unbridged_only(kz_term:ne_binary() | 'undefined', kz_json:object()) ->
    kz_term:ne_binary().
...
intercept_unbridged_only(ExistingCallId, JObj) ->
    case kz_json:is_true(<<"Intercept-Unbridged-Only">>, JObj, 'true') of
        'true' ->
            <<" 'set:intercept_unbridged_only=true,intercept:", ExistingCallId/binary,
                "' inline ">>;
        'false' ->
            <<" 'set:intercept_unbridged_only=false,intercept:", ExistingCallId/binary,
                "' inline ">>
    end.
...
-spec build_originate_args(kz_term:ne_binary(), state(), kz_json:object(), kz_term:ne_binary()) ->
    kz_term:api_binary().
build_originate_args(Action, State, JObj, FetchId) ->
    case kz_json:get_value(<<"Endpoints">>, JObj, []) of
        ...
        [Endpoint] ->
            ...
            build_originate_args_from_endpoints(
                Action, [update_endpoint(Endpoint, State)], JObj, FetchId
            );
        Endpoints ->
...
-spec build_originate_args_from_endpoints(
    kz_term:ne_binary(), kz_json:objects(), kz_json:object(), kz_term:ne_binary()
) ->
    kz_term:ne_binary().
...
get_channel_vars(JObj, FetchId) ->
    InteractionId = kz_json:get_value(
        [<<"Custom-Channel-Vars">>, <<?CALL_INTERACTION_ID>>], JObj, ?CALL_INTERACTION_DEFAULT
    ),
    CCVs = [
        {<<"Fetch-ID">>, FetchId},
...
set_music_on_hold(_, _, 'undefined') ->
    'ok';
...
get_unset_vars(JObj) ->
    ...
    FSVars = lists:foldr(
        fun ecallmgr_fs_xml:kazoo_var_to_fs_var/2,
        [],
    ...
    case
        [
            "unset:" ++ K
         || KV <- lists:foldr(
                fun ecallmgr_fs_xml:kazoo_var_to_fs_var/2, [], kz_json:to_proplist(JObj)
            ),
            not lists:member(
                begin
                    [K, _] = string:tokens(binary_to_list(KV), "="),
                    K
                end,
                Export
            )
...
maybe_fix_group_confirm(Export) ->
    case lists:member("group_confirm_key", Export) of
        'true' ->
            "";
        'false' ->
            "^unset:group_confirm_key^unset:group_confirm_cancel_timeout^unset:group_confirm_file"
    end.
...
maybe_fix_caller_id(Export, JObj) ->
    ...
    string:join(
        [
            "^set:" ++ Key ++ "=" ++ erlang:binary_to_list(Value)
         || {IsTrue, Value, Key} <- Fix, IsTrue
        ],
        ":"
    ).
...
-spec publish_error(
    kz_term:ne_binary(),
    created_uuid() | kz_term:api_binary(),
    kz_json:object(),
    kz_term:api_binary()
) -> 'ok'.
publish_error(_, _, _, 'undefined') ->
    'ok';
...
-spec publish_originate_ready(
    kz_term:ne_binary(),
    created_uuid() | kz_term:ne_binary(),
    kz_json:object(),
    kz_term:api_binary(),
    kz_term:api_binary()
) -> 'ok'.
...
publish_originate_ready(CtrlQ, UUID, Request, Q, ServerId) ->
    ...
    Props = [
        {<<"Msg-ID">>, kz_api:msg_id(Request, UUID)},
        {<<"Call-ID">>, UUID},
        {<<"Control-Queue">>, CtrlQ}
...
publish_originate_resp('undefined', _) ->
    'ok';
publish_originate_resp(ServerId, JObj) ->
    Resp = kz_json:set_values(
        [
...
publish_originate_resp('undefined', _JObj, _UUID) ->
    'ok';
...
-spec publish_originate_started(
    kz_term:api_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()
) -> 'ok'.
publish_originate_started('undefined', _, _, _) ->
    'ok';
...
-spec publish_originate_uuid(
    kz_term:api_binary(),
    created_uuid() | kz_term:ne_binary(),
    kz_json:object(),
    kz_term:ne_binary()
) -> 'ok'.
publish_originate_uuid('undefined', _, _, _) ->
    'ok';
...
maybe_send_originate_uuid({_, UUID}, Pid, #state{
    server_id = ServerId,
    originate_req = JObj
}) ->
    CtlQ = call_control_queue(Pid),
    publish_originate_uuid(ServerId, UUID, JObj, CtlQ).

-spec call_control_queue(pid()) -> kz_term:ne_binary().
call_control_queue(Pid) ->
    call_control_queue(Pid, ecallmgr_call_control:queue_name(Pid)).

-spec call_control_queue(pid(), kz_term:api_ne_binary()) -> kz_term:ne_binary().
call_control_queue(_Pid, <<QueueName/binary>>) ->
    QueueName;
call_control_queue(Pid, 'undefined') ->
    lager:debug("waiting on call control ~p to get setup", [Pid]),
    timer:sleep(100),
    call_control_queue(Pid).
...
update_endpoint(
    ...
    case uuid_matches(UUID, GlobalUUID) of
        'true' ->
            'ok';
        'false' ->
            maybe_start_call_handlers(UUID, State#state{
                uuid = UUID,
                control_pid = 'undefined'
            })
    end,

    EP = kz_json:set_values([{<<"origination_uuid">>, Id}], Endpoint),
    fix_hold_media(EP).
...
should_update_uuid(OldUUID, Props) ->
    case props:get_value(<<"Event-Subclass">>, Props, props:get_value(<<"Event-Name">>, Props)) of
        <<"loopback::bowout">> ->
            lager:debug(
                "bowout detected with ~s, old uuid is ~s",
