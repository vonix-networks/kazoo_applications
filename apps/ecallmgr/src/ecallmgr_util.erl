...
-export([
    media_path/1, media_path/2, media_path/3, media_path/4,
    moh_media_path/4,
    lookup_media/4
]).
...
-record(bridge_endpoint, {
    invite_format = <<"username">> :: kz_term:ne_binary(),
    ...
    % for Skype
    interface = <<"RR">> :: kz_term:ne_binary(),
    sip_interface,
...
get_expires(Props) ->
    Expiry = kz_term:to_integer(
        props:get_first_defined([<<"Expires">>, <<"expires">>], Props, 300)
    ),
    round(Expiry * 1.25).
...
get_interface_properties(Node) ->
    [
        {Interface, get_interface_properties(Node, Interface)}
     || Interface <- get_interface_list(Node)
    ].
...
get_interface_properties(Node, Interface) ->
    case
        freeswitch:api(
            Node, 'sofia', kz_term:to_list(list_to_binary(["status profile ", Interface]))
        )
    of
        {'ok', Response} ->
            R = binary:replace(Response, <<" ">>, <<>>, ['global']),
...
get_sip_to(Props, <<"outbound">>) ->
    case props:get_value(<<"Channel-Presence-ID">>, Props) of
        'undefined' ->
            Number = props:get_first_defined(
                [
...
get_sip_from(Props, _) ->
    Default = list_to_binary([
        kzd_freeswitch:from_user(Props),
        "@",
...
get_sip_interface_from_db([FsPath]) ->
    ...
    case map_fs_path_to_sip_profile(FsPath, NetworkMap) of
        'undefined' ->
            lager:debug(
                "unable to find network map for ~s, using default interface '~s'",
...
map_fs_path_to_sip_profile(FsPath, NetworkMap) ->
    SIPInterfaceObj = kz_json:filter(
        fun({K, _}) ->
...
-spec conference_channel_vars_fold({kz_term:ne_binary(), kz_term:ne_binary()}, kz_term:proplist()) ->
    kz_term:proplist().
...
-spec channel_var_map({kz_term:ne_binary(), kz_term:ne_binary()}) ->
...
-spec custom_channel_vars_fold({kz_term:ne_binary(), kz_term:ne_binary()}, kz_term:proplist()) ->
    kz_term:proplist().
...
custom_channel_vars_fold(_KV, Acc) ->
    Acc.
...
-spec custom_application_vars_fold({kz_term:ne_binary(), kz_term:ne_binary()}, kz_term:proplist()) ->
    kz_term:proplist().
...
custom_application_vars_fold({?GET_CAV_HEADER(Key), V}, Acc) ->
    props:insert_value(Key, V, Acc);
custom_application_vars_fold(_KV, Acc) ->
    Acc.
...
update_referred_by_ccv('undefined', CCVs) ->
...
update_referred_to_ccv('undefined', CCVs) ->
    props:delete(<<"Referred-To">>, CCVs);
...
unserialize_fs_array(List) when
    is_list(List)
->
    List;
...
get_setting(Setting) ->
    {'ok', kapps_config:get(?APP_NAME, Setting)}.
...
is_node_up(Node, UUID) ->
    ecallmgr_fs_nodes:is_node_up(Node) andalso
        ecallmgr_fs_channel:exists(UUID).
...
-spec multi_set_args(atom(), kz_term:ne_binary(), kz_term:proplist(), kz_term:ne_binary()) ->
    binary().
...
process_fs_kv(Node, UUID, [K | KVs], 'unset' = Action) when
    is_binary(K)
->
    X1 = get_fs_key(K),
    lists:foldl(
...
process_fs_kv_fold(_Node, _UUID, K, 'unset', Acc) when
    is_binary(K)
->
    Key = get_fs_key(K),
    [<<Key/binary, "=">> | Acc];
...
get_fs_kv(<<"Media-Names">>, [FileName1 | []], _) ->
    list_to_binary([
        get_fs_key(<<"Media-Names">>), "=", maybe_sanitize_fs_value(<<"Media-Names">>, FileName1)
    ]);
get_fs_kv(<<"Media-Recordings">>, [FileName1 | []], _) ->
    list_to_binary([
        get_fs_key(<<"Media-Recordings">>),
        "=",
        maybe_sanitize_fs_value(<<"Media-Recordings">>, FileName1)
    ]);
get_fs_kv(Key, Val, _) ->
...
get_fs_key(?CCV(Key)) ->
    get_fs_key(Key);
get_fs_key(?CAV(_) = CAV) ->
    CAV;
get_fs_key(<<"X-", _/binary>> = Key) ->
    <<"sip_h_", Key/binary>>;
get_fs_key(Key) ->
...
get_fs_key_and_value(Key, Val, _UUID) when
    is_binary(Val);
    is_atom(Val);
    is_number(Val);
    is_list(Val);
...
maybe_sanitize_fs_value(Key, Val) when
    Key =:= <<"Outbound-Caller-ID-Name">> orelse
        Key =:= <<"Outbound-Callee-ID-Name">> orelse
        Key =:= <<"Caller-ID-Name">> orelse
        Key =:= <<"Callee-ID-Name">>
->
    re:replace(Val, ?SANITIZE_FS_VALUE_REGEX, <<>>, [
        'ucp', 'global', 'unicode', {'return', 'binary'}
    ]);
...
-spec classify_endpoint(bridge_endpoint(), {bridge_endpoints(), bridge_endpoints()}) ->
...
endpoint_jobjs_to_records([Endpoint | Endpoints], IncludeVars, BridgeEndpoints) ->
    ...
    case
        kapi_dialplan:bridge_endpoint_v(Endpoint) andalso
            (not lists:keymember(Key, 1, BridgeEndpoints))
    of
        'false' ->
...
endpoint_jobj_to_record(Endpoint, IncludeVars) ->
    ...
    Bridge = #bridge_endpoint{
        invite_format = kz_json:get_ne_value(<<"Invite-Format">>, Endpoint, <<"username">>),
...
maybe_filter_failover_channels(Channels, 'false') ->
    Channels;
...
maybe_add_respond_header(Endpoint, OwnerId) ->
    case ecallmgr_fs_channels:has_channels_for_owner(OwnerId) of
        'true' ->
            ...
            kz_json:set_value(
                [<<"Custom-SIP-Headers">>, <<"X-KAZOO-Respond-With">>],
                <<"486 User Busy">>,
                Endpoint
            );
        'false' ->
...
build_bridge_channels(
    [#bridge_endpoint{invite_format = <<"route">>} = Endpoint | Endpoints], Channels
) ->
...
build_bridge_channels(
    [#bridge_endpoint{invite_format = <<"loopback">>} = Endpoint | Endpoints], Channels
) ->
    case build_channel(Endpoint) of
        {'error', _} -> build_bridge_channels(Endpoints, Channels);
        {'ok', Channel} -> build_bridge_channels(Endpoints, [Channel | Channels])
...
build_bridge_channels(
    [#bridge_endpoint{ip_address = 'undefined'} = Endpoint | Endpoints], Channels
) ->
    S = self(),
...
maybe_collect_worker_channel(Pid, Channels) ->
    ...
    after 2 * ?MILLISECONDS_IN_SECOND -> Channels
    end.
...
build_freetdm_channel(#bridge_endpoint{number = 'undefined'}) ->
    {'error', 'number_not_provided'};
build_freetdm_channel(#bridge_endpoint{
    invite_format = <<"e164">>,
    number = Number,
    span = Span,
    ...
    {'ok',
        <<"freetdm/", Span/binary, "/", ChannelSelection/binary, "/",
            (knm_converters:normalize(Number))/binary>>};
build_freetdm_channel(#bridge_endpoint{
    invite_format = <<"npan">>,
    number = Number,
    span = Span,
    ...
    {'ok',
        <<"freetdm/", Span/binary, "/", ChannelSelection/binary, "/",
            (knm_converters:to_npan(Number))/binary>>};
build_freetdm_channel(#bridge_endpoint{
    invite_format = <<"1npan">>,
    number = Number,
    span = Span,
    ...
    {'ok',
        <<"freetdm/", Span/binary, "/", ChannelSelection/binary, "/",
            (knm_converters:to_1npan(Number))/binary>>};
build_freetdm_channel(#bridge_endpoint{
    number = Number,
    span = Span,
    channel_selection = ChannelSelection
...
-type build_channel_fun_1() :: fun(
    (bridge_endpoint()) -> bridge_channel() | {bridge_channel(), bridge_endpoint()}
).
-type build_channel_fun_2() :: fun(
    (bridge_channel(), bridge_endpoint()) ->
        bridge_channel() | {bridge_channel(), bridge_endpoint()}
).
...
get_sip_contact(#bridge_endpoint{invite_format = <<"loopback">>, route = Route}) ->
    <<"loopback/", Route/binary, "/", (?DEFAULT_FREESWITCH_CONTEXT)/binary>>;
get_sip_contact(#bridge_endpoint{
...
maybe_clean_contact(<<"sip:", Contact/binary>>, _Endpoint) ->
    Contact;
maybe_clean_contact(<<"sips:", Contact/binary>>, _Endpoint) ->
    Contact;
...
maybe_replace_fs_path(Contact, #bridge_endpoint{proxy_address = <<"sip:", _/binary>> = Proxy}) ->
    case
        re:replace(Contact, <<";fs_path=[^;?]*">>, <<";fs_path=", Proxy/binary>>, [
            {'return', 'binary'}
        ])
    of
        Contact ->
...
maybe_replace_fs_path(Contact, #bridge_endpoint{proxy_address = Proxy} = Endpoint) ->
    maybe_replace_fs_path(Contact, Endpoint#bridge_endpoint{
        proxy_address = <<"sip:", Proxy/binary>>
    }).
...
maybe_format_user(Contact, #bridge_endpoint{
...
maybe_append_channel_vars(Contact, #bridge_endpoint{
    include_channel_vars = 'false',
    header_vars = []
}) ->
...
-spec create_masquerade_event(kz_term:ne_binary(), kz_term:ne_binary(), boolean()) ->
    kz_term:ne_binary().
...
media_path(<<"http://", _/binary>> = URI, _Type, _UUID, _) ->
    get_fs_playback(URI);
media_path(<<"https://", _/binary>> = URI, _Type, _UUID, _) ->
    get_fs_playback(URI);
media_path(<<?HTTP_GET_PREFIX, _/binary>> = Media, _Type, _UUID, _) ->
    Media;
media_path(<<"\$", _/binary>> = Media, _Type, _UUID, _) ->
    Media;
...
fax_filename(UUID) ->
    ...
    filename:join([
        kapps_config:get_ne_binary(?APP_NAME, <<"fax_file_path">>, <<"/tmp/">>),
...
recording_filename(MediaName) ->
    ...
    RecordingName = filename:join([
        Directory,
        <<(kz_amqp_util:encode(RootName))/binary, Ext/binary>>
    ]),
    _ = kz_cache:store_local(
        ?ECALLMGR_UTIL_CACHE,
...
recording_directory(_RelativePath) ->
    kapps_config:get_ne_binary(?APP_NAME, <<"recording_file_path">>, <<"/tmp/">>).
...
maybe_playback_via_shout(URI) ->
    case
        filename:extension(URI) =:= <<".mp3">> andalso
            kapps_config:is_true(?APP_NAME, <<"use_shout">>, 'false')
    of
        'false' ->
...
request_media_url(MediaName, Type, CallId, JObj) ->
    ...
    case
        kz_amqp_worker:call_collect(
            kz_json:set_values(MsgProps, JObj),
...
maybe_add_expires_deviation('undefined') ->
    'undefined';
...
maybe_add_expires_deviation_ms('undefined') ->
    'undefined';
...
-spec get_dial_separator(kz_term:api_object() | kz_term:ne_binary(), kz_json:objects()) ->
    kz_term:ne_binary().
get_dial_separator(?DIAL_METHOD_SINGLE, _Endpoints) ->
    ?SEPARATOR_SINGLE;
get_dial_separator(?DIAL_METHOD_SIMUL, [_, _ | _]) ->
    ?SEPARATOR_SIMULTANEOUS;
get_dial_separator(?DIAL_METHOD_SIMUL, [_]) ->
    ?SEPARATOR_SINGLE;
get_dial_separator('undefined', _Endpoints) ->
    ?SEPARATOR_SINGLE;
get_dial_separator(JObj, Endpoints) ->
    get_dial_separator(
        kz_json:get_value(<<"Dial-Endpoint-Method">>, JObj, ?DIAL_METHOD_SINGLE),
        Endpoints
    ).

-spec fix_contact(kz_term:api_binary() | list(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_binary().
fix_contact('undefined', _, _) ->
    'undefined';
fix_contact(<<";", _/binary>> = OriginalContact, Username, Realm) ->
    fix_contact(
        <<"sip:", Username/binary, "@", Realm/binary, OriginalContact/binary>>, Username, Realm
    );
fix_contact(OriginalContact, Username, Realm) when
    is_binary(OriginalContact)
->
    fix_contact(
        binary:split(kz_binary:strip(OriginalContact), <<";">>, ['global']), Username, Realm
    );
...
fix_contact([Contact | Options], Username, Realm) ->
    case kzsip_uri:uris(Contact) of
        [#uri{user = <<>>, domain = <<>>} = Uri] ->
            fix_contact(
                [kzsip_uri:ruri(Uri#uri{user = Username, domain = Realm}) | Options],
                Username,
                Realm
            );
        [#uri{user = <<>>} = Uri] ->
...
-spec moh_media_path(kz_term:api_binary(), media_types(), kz_term:ne_binary(), kz_json:object()) ->
    kz_term:ne_binary().
