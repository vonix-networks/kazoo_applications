...
render_test_() ->
    {'setup', fun kz_fixturedb_util:start_me/0, fun kz_fixturedb_util:stop_me/1, fun(
        _ReturnOfSetup
    ) ->
        [
            ...
            {"HELP-44944: filtering of unregistered SIP endpoints with dedup",
...
no_filter_when_callfwd() ->
    ...
    Needles = [
        _DeviceId = <<"Authorizing-ID='device00000000000000000000000004'">>,
...
filter_endpoints_10722() ->
    ...
    UnregedEndpoint = kz_json:set_values(
        [
            {<<"To-User">>, <<"unregistered">>},
...
found(_Needle, []) ->
    ?debugFmt("needle ~s not found~n", [_Needle]),
    'false';
