            ...
            begin
                ...
                ?WHENFAIL(
                    begin
                        ...
                        ?debugFmt(
                            "~nLRU: ~p~n Final State: ~p~nZip:~n~s~nRecreate:~n~s~n",
                            [
                                _Pid,
                                State,
...
print_zip(Zip) ->
    ...
    {Print, Recreate} = lists:foldl(
        fun(Item, {Z, R}) -> {[print_zip_item(Item) | Z], [recreate_item(Item) | R]} end,
        {[], [StartStep, StopStep]},
        Zip
    ),
    {Print, [StopStep | Recreate]}.

recreate_item({{set, _Var, {call, M, F, As}}, {#model{cache = _Cache, now_ms = _NowMs}, Result}}) ->
    [io_lib:format("~p = ~s:~s(~s)", [Result, M, F, args_to_list(As)])].

print_zip_item({{set, _Var, {call, M, F, As}}, {#model{cache = Cache, now_ms = NowMs}, Result}}) ->
    [
...
command(#model{}) ->
    ...
    oneof([
        {'call', 'hon_trie', 'match_did', [phone_number(), 'undefined', ?KZ_RATES_DB]},
...
next_state(
    ...
    case find_prefix(Cache, PhoneNumber) of
        'error' ->
            Model#model{now_ms = NowMs + 1};
        {'ok', Prefix, RateIds} ->
            Model#model{
                cache = bump_matched(expire_rates(Cache, NowMs), NowMs, Prefix, RateIds),
                now_ms = NowMs + 1
            }
    end;
next_state(
...
postcondition(
    ...
    case find_prefix(Cache, PhoneNumber) of
        'error' ->
            'false';
        {'ok', _Prefix, RateIds} ->
            length(RateDocs) =:= length(RateIds) andalso
                lists:all(fun(RateId) -> props:is_defined(RateId, RateIds) end, RateDocs)
    end;
postcondition(
    #model{},
...
cache_rate(Rate, {Cache, NowMs}) ->
    ...
    {props:set_value(Prefix, NewRates, Cache), NowMs}.
...
expire_rate({Prefix, Rates}, {Cache, OldestTimestamp}) ->
    case
        lists:filter(
            fun({_RateId, LastUsed}) -> LastUsed >= OldestTimestamp end,
            Rates
        )
    of
...
find_prefix(Cache, PhoneNumber) ->
    ...
    case lists:foldl(fun longest_prefix/2, {PNBin, <<>>, 0, []}, Cache) of
        {_, <<>>, 0, []} -> 'error';
        {_, Prefix, _Len, Rates} -> {'ok', Prefix, Rates}
    end.
