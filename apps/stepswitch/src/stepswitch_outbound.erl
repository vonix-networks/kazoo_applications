...
handle_originate_req(OffnetReq) ->
    Number = stepswitch_util:get_outbound_destination(OffnetReq),
    lager:debug(
        "received outbound audio resource request for ~s from account ~s",
        [Number, kapi_offnet_resource:account_id(OffnetReq)]
    ),
    handle_originate_req(
        Number, maybe_add_call_id(kz_json:get_value(<<"Outbound-Call-ID">>, OffnetReq), OffnetReq)
    ).
...
maybe_add_call_id(_, OffnetReq) ->
    OffnetReq.

-spec maybe_force_originate_outbound(
    knm_number_options:extra_options(), kapi_offnet_resource:req()
) -> any().
maybe_force_originate_outbound(Props, OffnetReq) ->
    case
        knm_number_options:should_force_outbound(Props) orelse
            kz_json:is_true(<<"Force-Outbound">>, OffnetReq, 'false') orelse
            kz_term:is_ne_binary(kapi_offnet_resource:hunt_account_id(OffnetReq))
    of
...
maybe_force_outbound(Props, OffnetReq) ->
    case
        knm_number_options:should_force_outbound(Props) orelse
            kapi_offnet_resource:force_outbound(OffnetReq, 'false') orelse
            kapi_offnet_resource:hunt_account_id(OffnetReq) /= 'undefined'
    of
...
maybe_correct_shortdial(Number, OffnetReq) ->
    case stepswitch_util:correct_shortdial(Number, OffnetReq) of
        'undefined' ->
            lager:debug("no endpoints found for '~s', and no shortdial correction available", [
                Number
            ]),
        ...
        Number ->
            ...
            publish_no_resources(OffnetReq);
        CorrectedNumber ->
            lager:debug("corrected shortdial from '~s' to '~s', trying routing again", [
...
-spec local_originate_caller_id(kapi_offnet_resource:req()) ->
...
create_loopback_endpoint(Props, OffnetReq) ->
    ...
    CCVs = kz_json:from_list(
        [
            ...
            {
                <<?CHANNEL_LOOPBACK_HEADER_PREFIX, "Inception">>,
                <<Number/binary, "@", TargetRealm/binary>>
            },
            ...
            {
                <<?CHANNEL_LOOPBACK_HEADER_PREFIX, "From-URI">>,
                <<CIDNum/binary, "@", TargetRealm/binary>>
            },
            {
                <<?CHANNEL_LOOPBACK_HEADER_PREFIX, "Request-URI">>,
                <<Number/binary, "@", TargetRealm/binary>>
            },
            {
                <<?CHANNEL_LOOPBACK_HEADER_PREFIX, "To-URI">>,
                <<Number/binary, "@", TargetRealm/binary>>
            },
...
publish_no_resources(OffnetReq) ->
    case kapi_offnet_resource:server_id(OffnetReq) of
        'undefined' -> 'ok';
        ResponseQ -> kapi_offnet_resource:publish_resp(ResponseQ, no_resources(OffnetReq))
    end.
